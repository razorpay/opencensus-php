...
func getTokenAfterLogin() (string, string) {
	...
	escapedUserName := url.QueryEscape(adminDashboardTestCreds[USERNAME])
	escapedPassword := url.QueryEscape(adminDashboardTestCreds[PASSWORD])

	loginCredsData := USERNAME + "=" + escapedUserName + "&" + PASSWORD + "=" + escapedPassword

	if err != nil {
		return "", ""
	}

	req, err = http.NewRequest(POST_METHOD, e2e.Config.App.Hostname+PATH_ADMIN_SIGNIN, strings.NewReader(loginCredsData))

	if err != nil {
		return "", ""
	}

	req.Header.Set(HEADER_CONTENT_TYPE, APPLICATION_X_WWW_FORM_URLENCODED)
