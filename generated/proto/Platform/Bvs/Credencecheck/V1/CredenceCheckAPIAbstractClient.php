...
/**
 * @internal CredenceCheckAPIAbstractClient provides abstraction for JsonClient and Client (default).
 ...
 */
abstract class CredenceCheckAPIAbstractClient
{
    /**
     * @var server
     */
     ...
     * {@inheritdoc}
     */
    public function Create(array $ctx, \Platform\Bvs\Credencecheck\V1\CreateRequest $in): \Platform\Bvs\Credencecheck\V1\CreateResponse
    {
        $ctx = Context::withPackageName($ctx, 'platform.bvs.credencecheck.v1');
        $ctx = Context::withServiceName($ctx, 'CredenceCheckAPI');
        $ctx = Context::withMethodName($ctx, 'Create');

        $out = new \Platform\Bvs\Credencecheck\V1\CreateResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/platform.bvs.credencecheck.v1.CredenceCheckAPI/Create';
        } else {
            $url = $url.'/'.$this->prefix.'/platform.bvs.credencecheck.v1.CredenceCheckAPI/Create';
        }
     ...
     * {@inheritdoc}
     */
    public function GetDetailsById(array $ctx, \Platform\Bvs\Credencecheck\V1\GetDetailsByIdRequest $in): \Platform\Bvs\Credencecheck\V1\GetDetailsByIdResponse
    {
        $ctx = Context::withPackageName($ctx, 'platform.bvs.credencecheck.v1');
        $ctx = Context::withServiceName($ctx, 'CredenceCheckAPI');
        $ctx = Context::withMethodName($ctx, 'GetDetailsById');

        $out = new \Platform\Bvs\Credencecheck\V1\GetDetailsByIdResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/platform.bvs.credencecheck.v1.CredenceCheckAPI/GetDetailsById';
        } else {
            $url = $url.'/'.$this->prefix.'/platform.bvs.credencecheck.v1.CredenceCheckAPI/GetDetailsById';
        }
     ...
     * {@inheritdoc}
     */
    public function GetDetailsByAccountId(array $ctx, \Platform\Bvs\Credencecheck\V1\GetDetailsByAccountIdRequest $in): \Platform\Bvs\Credencecheck\V1\GetDetailsByAccountIdResponse
    {
        $ctx = Context::withPackageName($ctx, 'platform.bvs.credencecheck.v1');
        $ctx = Context::withServiceName($ctx, 'CredenceCheckAPI');
        $ctx = Context::withMethodName($ctx, 'GetDetailsByAccountId');

        $out = new \Platform\Bvs\Credencecheck\V1\GetDetailsByAccountIdResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/platform.bvs.credencecheck.v1.CredenceCheckAPI/GetDetailsByAccountId';
        } else {
            $url = $url.'/'.$this->prefix.'/platform.bvs.credencecheck.v1.CredenceCheckAPI/GetDetailsByAccountId';
        }
