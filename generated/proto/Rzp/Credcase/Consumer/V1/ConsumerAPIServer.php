 ...
 */
final class ConsumerAPIServer implements RequestHandlerInterface
{
    const PATH_PREFIX = '/twirp/rzp.credcase.consumer.v1.ConsumerAPI/';
    ...
    public function handle(ServerRequestInterface $req): ResponseInterface
    {
        ...
        $ctx = Context::withPackageName($ctx, 'rzp.credcase.consumer.v1');
        $ctx = Context::withServiceName($ctx, 'ConsumerAPI');
        ...
        switch ($req->getUri()->getPath()) {
            case '/twirp/rzp.credcase.consumer.v1.ConsumerAPI/CreateConsumer':
                return $this->handleCreateConsumer($ctx, $req);
            case '/twirp/rzp.credcase.consumer.v1.ConsumerAPI/DeleteConsumer':
                return $this->handleDeleteConsumer($ctx, $req);
    ...
    private function handleCreateConsumer(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleCreateConsumerJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleCreateConsumerProtobuf($ctx, $req);
                break;
    ...
    private function handleCreateConsumerJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'CreateConsumer');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Rzp\Credcase\Consumer\V1\Consumer();
            $in->mergeFromJsonString((string)$req->getBody());

            $out = $this->svc->CreateConsumer($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling CreateConsumer. null responses are not supported'));
            }
    ...
    private function handleCreateConsumerProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'CreateConsumer');
    ...
    private function handleDeleteConsumer(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $header = $req->getHeaderLine('Content-Type');
        $i = strpos($header, ';');
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleDeleteConsumerJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleDeleteConsumerProtobuf($ctx, $req);
                break;
    ...
    private function handleDeleteConsumerJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'DeleteConsumer');
        ...
        try {
            ...
            $out = $this->svc->DeleteConsumer($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling DeleteConsumer. null responses are not supported'));
            }
    ...
    private function handleDeleteConsumerProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'DeleteConsumer');
