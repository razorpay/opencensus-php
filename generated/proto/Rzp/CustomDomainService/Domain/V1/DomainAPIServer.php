 ...
 */
final class DomainAPIServer implements RequestHandlerInterface
{
    /**
     ...
     *
     * Use DomainAPIServer::getPathPrefix instead.
     */
    public const PATH_PREFIX = '/twirp/rzp.custom_domain_service.domain.v1.DomainAPI/';
    ...
    {
        return $this->prefix.'/rzp.custom_domain_service.domain.v1.DomainAPI/';
    }
    ...
    public function handle(ServerRequestInterface $req): ResponseInterface
    {
        ...
        if ($service != 'rzp.custom_domain_service.domain.v1.DomainAPI') {
        ...
        switch ($method) {
            ...
            case 'IsSubDomain':
                return $this->handleIsSubDomain($ctx, $req);
    ...
    private function handleCreateJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        try {
            ...
            $in = new \Rzp\CustomDomainService\Domain\V1\DomainRequest();
    ...
    private function handleListJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        try {
            ...
            $in = new \Rzp\CustomDomainService\Domain\V1\ListDomainRequest();
            $in->mergeFromJsonString((string)$req->getBody(), true);
    ...
    private function handleIsSubDomain(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $header = $req->getHeaderLine('Content-Type');
        $i = strpos($header, ';');
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleIsSubDomainJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleIsSubDomainProtobuf($ctx, $req);
                break;
    ...
    private function handleIsSubDomainJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        try {
            ...
            $out = $this->svc->IsSubDomain($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling IsSubDomain. null responses are not supported'));
            }
    ...
    private function handleIsSubDomainProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'IsSubDomain');
