...
/**
 * @see SyncDeviationAPI
 *
 * Generated from protobuf service <code>rzp.accounts.sync_deviation.v1.SyncDeviationAPI</code>
 */
final class SyncDeviationAPIServer implements RequestHandlerInterface
{
    /**
     ...
     *
     * Use SyncDeviationAPIServer::getPathPrefix instead.
     */
    public const PATH_PREFIX = '/twirp/rzp.accounts.sync_deviation.v1.SyncDeviationAPI/';
    ...
    {
        return $this->prefix.'/rzp.accounts.sync_deviation.v1.SyncDeviationAPI/';
    }
    ...
    public function handle(ServerRequestInterface $req): ResponseInterface
    {
        ...
        $ctx = Context::withPackageName($ctx, 'rzp.accounts.sync_deviation.v1');
        $ctx = Context::withServiceName($ctx, 'SyncDeviationAPI');
        ...
        if ($service != 'rzp.accounts.sync_deviation.v1.SyncDeviationAPI') {
        ...
        switch ($method) {
            case 'SyncAccountDeviation':
                return $this->handleSyncAccountDeviation($ctx, $req);
    ...
    private function handleSyncAccountDeviation(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleSyncAccountDeviationJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleSyncAccountDeviationProtobuf($ctx, $req);
                break;
    ...
    private function handleSyncAccountDeviationJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'SyncAccountDeviation');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Rzp\Accounts\SyncDeviation\V1\SyncAccountDeviationRequest();
            $in->mergeFromJsonString((string)$req->getBody(), true);

            $out = $this->svc->SyncAccountDeviation($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling SyncAccountDeviation. null responses are not supported'));
            }
    ...
    private function handleSyncAccountDeviationProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'SyncAccountDeviation');
