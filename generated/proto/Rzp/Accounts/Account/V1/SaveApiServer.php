 ...
 */
final class SaveApiServer implements RequestHandlerInterface
{
    /**
     ...
     *
     * Use SaveApiServer::getPathPrefix instead.
     */
    public const PATH_PREFIX = '/twirp/rzp.accounts.account.v1.SaveApi/';
    ...
    {
        return $this->prefix.'/rzp.accounts.account.v1.SaveApi/';
    }
    ...
    public function handle(ServerRequestInterface $req): ResponseInterface
    {
        ...
        $ctx = Context::withServiceName($ctx, 'SaveApi');
        ...
        if ($service != 'rzp.accounts.account.v1.SaveApi') {
        ...
        switch ($method) {
            case 'Save':
                return $this->handleSave($ctx, $req);
    ...
    private function handleSave(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleSaveJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleSaveProtobuf($ctx, $req);
                break;
    ...
    private function handleSaveJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'Save');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Rzp\Accounts\Account\V1\SaveAccountEntityRequest();
            $in->mergeFromJsonString((string)$req->getBody(), true);

            $out = $this->svc->Save($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling Save. null responses are not supported'));
            }
    ...
    private function handleSaveProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'Save');
