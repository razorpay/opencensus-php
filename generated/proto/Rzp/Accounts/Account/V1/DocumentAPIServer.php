 ...
 */
final class DocumentAPIServer implements RequestHandlerInterface
{
    /**
     ...
     *
     * Use DocumentAPIServer::getPathPrefix instead.
     */
    public const PATH_PREFIX = '/twirp/rzp.accounts.account.v1.DocumentAPI/';
    ...
    {
        return $this->prefix.'/rzp.accounts.account.v1.DocumentAPI/';
    }
    ...
    public function handle(ServerRequestInterface $req): ResponseInterface
    {
        ...
        $ctx = Context::withServiceName($ctx, 'DocumentAPI');
        ...
        if ($service != 'rzp.accounts.account.v1.DocumentAPI') {
        ...
        switch ($method) {
            ...
            case 'FetchMerchantDocuments':
                return $this->handleFetchMerchantDocuments($ctx, $req);
    ...
    private function handleCreateJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        try {
            ...
            $in = new \Rzp\Accounts\Account\V1\CreateDocumentsRequest();
    ...
    private function handleFetchJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        try {
            ...
            $in = new \Rzp\Accounts\Account\V1\FetchDocumentsRequest();
            $in->mergeFromJsonString((string)$req->getBody(), true);
    ...
    private function handleDeleteJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        try {
            ...
            $in = new \Rzp\Accounts\Account\V1\DeleteDocumentRequest();
            $in->mergeFromJsonString((string)$req->getBody(), true);
    ...
    private function handleFetchMerchantDocuments(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $header = $req->getHeaderLine('Content-Type');
        $i = strpos($header, ';');
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleFetchMerchantDocumentsJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleFetchMerchantDocumentsProtobuf($ctx, $req);
                break;
    ...
    private function handleFetchMerchantDocumentsJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'FetchMerchantDocuments');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Rzp\Accounts\Account\V1\FetchMerchantDocumentsRequest();
            $in->mergeFromJsonString((string)$req->getBody(), true);

            $out = $this->svc->FetchMerchantDocuments($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling FetchMerchantDocuments. null responses are not supported'));
            }
    ...
    private function handleFetchMerchantDocumentsProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'FetchMerchantDocuments');
