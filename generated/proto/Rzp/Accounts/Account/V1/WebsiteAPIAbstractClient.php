...
/**
 * @internal WebsiteAPIAbstractClient provides abstraction for JsonClient and Client (default).
 ...
 */
abstract class WebsiteAPIAbstractClient
{
    /**
     * @var server
     */
     ...
     * {@inheritdoc}
     */
    public function Fetch(array $ctx, \Rzp\Accounts\Account\V1\FetchAccountWebsiteRequest $in): \Rzp\Accounts\Account\V1\FetchAccountWebsiteResponse
    {
        $ctx = Context::withPackageName($ctx, 'rzp.accounts.account.v1');
        $ctx = Context::withServiceName($ctx, 'WebsiteAPI');
        $ctx = Context::withMethodName($ctx, 'Fetch');

        $out = new \Rzp\Accounts\Account\V1\FetchAccountWebsiteResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/rzp.accounts.account.v1.WebsiteAPI/Fetch';
        } else {
            $url = $url.'/'.$this->prefix.'/rzp.accounts.account.v1.WebsiteAPI/Fetch';
        }
     ...
     * {@inheritdoc}
     */
    public function FetchMerchantWebsite(array $ctx, \Rzp\Accounts\Account\V1\FetchAccountWebsiteRequest $in): \Rzp\Accounts\Account\V1\FetchMerchantWebsiteResponse
    {
        $ctx = Context::withPackageName($ctx, 'rzp.accounts.account.v1');
        $ctx = Context::withServiceName($ctx, 'WebsiteAPI');
        $ctx = Context::withMethodName($ctx, 'FetchMerchantWebsite');

        $out = new \Rzp\Accounts\Account\V1\FetchMerchantWebsiteResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/rzp.accounts.account.v1.WebsiteAPI/FetchMerchantWebsite';
        } else {
            $url = $url.'/'.$this->prefix.'/rzp.accounts.account.v1.WebsiteAPI/FetchMerchantWebsite';
        }
