 ...
 */
final class WebsiteAPIServer implements RequestHandlerInterface
{
    /**
     ...
     *
     * Use WebsiteAPIServer::getPathPrefix instead.
     */
    public const PATH_PREFIX = '/twirp/rzp.accounts.account.v1.WebsiteAPI/';
    ...
    {
        return $this->prefix.'/rzp.accounts.account.v1.WebsiteAPI/';
    }
    ...
    public function handle(ServerRequestInterface $req): ResponseInterface
    {
        ...
        if ($service != 'rzp.accounts.account.v1.WebsiteAPI') {
        ...
        switch ($method) {
            ...
            case 'FetchMerchantWebsite':
                return $this->handleFetchMerchantWebsite($ctx, $req);
    ...
    private function handleFetchJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        try {
            ...
            $in = new \Rzp\Accounts\Account\V1\FetchAccountWebsiteRequest();
    ...
    private function handleFetchMerchantWebsite(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $header = $req->getHeaderLine('Content-Type');
        $i = strpos($header, ';');
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleFetchMerchantWebsiteJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleFetchMerchantWebsiteProtobuf($ctx, $req);
                break;
    ...
    private function handleFetchMerchantWebsiteJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        try {
            ...
            $out = $this->svc->FetchMerchantWebsite($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling FetchMerchantWebsite. null responses are not supported'));
            }
    ...
    private function handleFetchMerchantWebsiteProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'FetchMerchantWebsite');
