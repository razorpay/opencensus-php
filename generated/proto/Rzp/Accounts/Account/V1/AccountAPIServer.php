 ...
 */
final class AccountAPIServer implements RequestHandlerInterface
{
    /**
     ...
     *
     * Use AccountAPIServer::getPathPrefix instead.
     */
    public const PATH_PREFIX = '/twirp/rzp.accounts.account.v1.AccountAPI/';
    ...
    {
        return $this->prefix.'/rzp.accounts.account.v1.AccountAPI/';
    }
    ...
    public function handle(ServerRequestInterface $req): ResponseInterface
    {
        ...
        $ctx = Context::withPackageName($ctx, 'rzp.accounts.account.v1');
        $ctx = Context::withServiceName($ctx, 'AccountAPI');
        ...
        if ($service != 'rzp.accounts.account.v1.AccountAPI') {
        ...
        switch ($method) {
            ...
            case 'Fetch':
                return $this->handleFetch($ctx, $req);
            case 'DeleteAccountContact':
                return $this->handleDeleteAccountContact($ctx, $req);
    ...
    private function handleCreateJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        try {
            ...
            $in = new \Rzp\Accounts\Account\V1\CreateAccountRequest();
    ...
    private function handleUpdateJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        try {
            ...
            $in = new \Rzp\Accounts\Account\V1\UpdateAccountRequest();
            $in->mergeFromJsonString((string)$req->getBody(), true);
    ...
    private function handleFetch(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $header = $req->getHeaderLine('Content-Type');
        $i = strpos($header, ';');
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleFetchJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleFetchProtobuf($ctx, $req);
                break;
    ...
    private function handleFetchJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'Fetch');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Rzp\Accounts\Account\V1\FetchAccountRequest();
            $in->mergeFromJsonString((string)$req->getBody(), true);

            $out = $this->svc->Fetch($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling Fetch. null responses are not supported'));
            }
    ...
    private function handleFetchProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'Fetch');
    ...
    private function handleDeleteAccountContact(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $header = $req->getHeaderLine('Content-Type');
        $i = strpos($header, ';');
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleDeleteAccountContactJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleDeleteAccountContactProtobuf($ctx, $req);
                break;
    ...
    private function handleDeleteAccountContactJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'DeleteAccountContact');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Rzp\Accounts\Account\V1\DeleteAccountContactRequest();
            $in->mergeFromJsonString((string)$req->getBody(), true);

            $out = $this->svc->DeleteAccountContact($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling DeleteAccountContact. null responses are not supported'));
            }
    ...
    private function handleDeleteAccountContactProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'DeleteAccountContact');
