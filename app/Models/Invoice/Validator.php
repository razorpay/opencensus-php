    ...
    */
    ...
    public function validateReceipt(string $attribute, string $receipt)
    {
        if (empty($receipt) === false)
        {
            ...
            if($this->entity->isPaymentPageInvoice() === true or $this->entity->isNCAPaymentPageInvoice() === true or $this->entity->isTypeOPGSPInvoice() === true)
            {
    ...
    protected function validateMerchantIsNotFeeBearer(Merchant\Entity $merchant, Entity $invoice)
    {
         ...
         */
        if ($invoice->isPaymentPageInvoice() === true || $invoice->isNCAPaymentPageInvoice() === true)
        {
            return;
        }
    ...
    public function validateSendNotificationRequest(string $medium)
    {
        ...
        $op = $invoice->isPaymentPageInvoice() || $invoice->isNCAPaymentPageInvoice() ? 'sendPPReceiptNotification' : $op;
