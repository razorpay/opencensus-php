...
class ExtendedValidations extends \Razorpay\Spine\Validation\LaravelValidatorEx
{
    ...
    // Overriding for MPI CTH
    protected function validateNonPastYear($attribute, $value)
    {
        $year = $value;
    ...
    protected function validateIfPresentThen($attribute, $value, $parameters)
    {
         $this->requireParameterCount(2, $parameters, 'if_present_then');

         $data = array_get($this->data, $parameters[0]);

         $value = $parameters[1];

         if (($data === null) or ($data !== $value))
         {
             return false;
         }
    ...
    protected function validateSizeIn($attribute, $value, $parameters)
    {
        $size = (string) $this->getSize($attribute, $value);

        if (in_array($size, $parameters, true))
        {
            return true;
        }
    ...
    protected function validateSizeWoWhitespace($attribute, $value, $parameters)
    {
        $trimmedValue = str_replace(' ', '', $value);

        return $this->validateSize($attribute, $trimmedValue, $parameters);
    }

    protected function validateVersion($attribute, $value, $parameters)
    {
        $this->requireParameterCount(2, $parameters, 'version');

        $comparisonVersion = $parameters[0];
        $operator          = $parameters[1];

        if (version_compare($value, $comparisonVersion, $operator) === false)
        {
            throw new Exception\BadRequestValidationFailureException(
                    'Invalid version provided', 'version', $value);
        }

        return true;
    }

    protected function validateUrlScheme($attribute, $value, $parameters)
    {
        $urlScheme = parse_url($value, PHP_URL_SCHEME);

        return in_array($urlScheme, $parameters, true);
    }

    protected function validateCavv($attribute, $value, $parameters)
    {
        $trimmedValue = strtr($value, ' ', '');

        return (mb_strlen(base64_decode($trimmedValue)) === 20);
    }

    protected function validateCritical($attribute, $value)
    {
        return ($value === 'false');
    }
}
