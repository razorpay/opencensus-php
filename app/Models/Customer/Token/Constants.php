...
class Constants
{
    ...
    const DATA_LAKE_TOKEN_HQ_PENDING_MERCHANT = "SELECT rtb.merchant_id, terminals.org_id, terminals.gateway FROM hive.aggregate_pa.rtb_eligibility_merchants_transactions_v1 rtb LEFT JOIN realtime_terminalslive.terminals terminals ON rtb.merchant_id = terminals.merchant_id AND terminals.status != 'activated' AND terminals.deleted_at IS NULL WHERE rtb.count_successful >= 1 AND terminals.gateway IN ('tokenisation_visa', 'tokenisation_mastercard', 'tokenisation_rupay')";
