...
class UpiAirtel extends Base
{

    const FILE_NAME       = 'AirtelRefund';
    const EXTENSION       = FileStore\Format::XLSX;
    const FILE_TYPE       = FileStore\Type::AIRTEL_UPI_REFUND;
    const GATEWAY         = Payment\Gateway::UPI_AIRTEL;
    const BEAM_FILE_TYPE  = 'refund';

    const BASE_STORAGE_DIRECTORY = 'upi/upi_airtel/refund/normal_refund_file/';
    ...
    protected function formatDataForFile(array $data)
    {
        ...
        {
            $orgRRN                 = trim(RefundConstants::ORG_RRN, '"');
            $dateAndTime            = trim(RefundConstants::DATE_AND_TIME, '"');
            $bankOrgTransactionId   = trim(RefundConstants::BANK_ORG_TRANSACTION_ID, '"');
            $orgAmount              = trim(RefundConstants::ORG_AMOUNT, '"');
            $refundAmt              = trim(RefundConstants::REFUND_AMOUNT, '"');
            $refundStatus           = trim(RefundConstants::REFUND_STATUS, '"');
            $refundReason           = trim(RefundConstants::REFUND_REASON, '"');
            $refundId               = trim(RefundConstants::REFUND_ID,'"');

            $status = $row['payment']['refund_status'];
            ...
            $npciTxnId = $row['gateway']['gateway_payment_id'] ?? $row['gateway']['gateway_reference'] ?? '';
            ...
            $formattedData[] = [
                $orgRRN               => trim($npciReferenceId, '"'),
                $dateAndTime          => Carbon::createFromTimestamp($row['payment']['created_at'], Timezone::IST)->format('m/d/Y'),
                $bankOrgTransactionId => trim($npciTxnId, '"'),
                $orgAmount            => trim($paymentAmount, '"'),
                $refundAmt            => trim($refundAmount, '"'),
                $refundStatus         => $status,
