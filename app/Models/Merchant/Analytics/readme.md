...
- For each aggregation in the response:
    - If the aggregation name is related to overall conversion rate (CR), the `calculateOverallCR` function is called to calculate the CR for each data point in the aggregation.
...
This function calculates the Overall Conversion Rate (CR) for each data point in the given aggregation.

**Parameters**
- `data`: The data points to calculate the Overall CR for.
...
**Return Value**
An array containing the calculated overall CR for each data point.
...
- Iterate over each data point:
  ...
  - Update the total checkout renders count using the `updateTotalCheckoutRendersCount` function.
      - This function based on group key groups the count of total number of checkout renders.
      - For the above example
          - The `total_checkout_renders` will be `value(count of occurrences)` => `5`.
...
$groupedCounts = [
    '1684956600-card' => [
        ...
        'total_checkout_renders' => 15
    ],
    '1684960200-wallet' => [
        'number_of_successful_payments' => 18,
        'total_checkout_renders' => 36
    ],
    '1684967400-wallet' => [
        'number_of_successful_payments' => 0,
        'total_checkout_renders' => 18
    ]
];
$otherFields = [
    '1684956600-card' => [
...
```
- Divide the grouped counts by the number of successful payments and the total number of checkout renders and multiplies by 100 using the `calculatePercentage` function.
