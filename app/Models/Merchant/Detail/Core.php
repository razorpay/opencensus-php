...
class Core extends Base\Core
{
                                        ...
                                        string $originProduct = Product::PRIMARY)
    {
        ...
        Still not pushing any message to queue. This can be reverted if not needed.
         */
    ...
    public function submitActivationForm(Merchant\Entity $merchant, array $input = null, string $originProduct = Product::PRIMARY, $orgId = null)
    {
        ...
        $submittedAt = $merchant->merchantDetail->getSubmittedAt();
        ...
        // the activation status above should be always under review if fee based gating is eligible

        if ((empty($statusToBeUpdated) === false) and ($isFeeBasedGatingEligible === false))
        {
        ...
        {
            $response[DetailConstants::FEE_BASED_GATING] = [
                DetailConstants::IS_ELIGIBLE => true
            ];
        }
    ...
    public function updateActivationStatus(Merchant\Entity $merchant, array $input, PublicEntity $maker, bool $triggerWorkflow = true): Entity
    {
        ...
        $this->repo->transactionOnLiveAndTestAndAsv(function() use (
            ...
            if (empty($status) === false && $this->shouldTriggerActivatedWebhook($merchant->getId(), $status))
            {
                $eventPayload = [
    ...
    protected function isMerchantEligibleToRemoveNGO($merchantDetail)
    {
        $isExpEnabled = (new Merchant\Core)->isSplitzExperimentEnable(
            [
                'id'            =>  $merchantDetail->getMerchantId(),
                'experiment_id' => $this->app['config']->get('app.remove_ngo_business_type'),
            ],
            'variables'
        );
    ...
    public function submitSalesAssistedActivationForm(string $merchantId): array
    {
        try
        {
            $merchantDetails = $this->repo->merchant_detail->findOrFail($merchantId);
    ...
    private function appendBankingSpecificDetails(array $response, Merchant\Entity $merchant): array
    {
        ...
        if (empty($balance) === false)
        {
            ...
            {
                $ledgerResponse = (new LedgerCore())->fetchBalanceFromLedger($merchant, $bankingAccount->getPublicId());
...
//              If the merchant is sales assisted skip adding individual or not_yet_registered as business type
    ...
    public function isEligibleForFeeBasedGating(Merchant\Entity $merchant, MerchantDetailEntity $merchantDetails): bool
    {
        ...
        $merchantOrg = $merchant->getOrgId();

        // individual or unregistered business type
        $merchantBusinessType = $merchantDetails->getBusinessType();

        //any website should not be there
    ...
    protected function saveFeeBasedGatingEligibilityToPgos(Merchant\Entity $merchant, bool $isFeeBasedGatingEligible = false)
    {
        ...
        if ($pgosMock === true)
        {
            $mockResponse[Constant::SUCCESS] = true;
        ...
        try
        {
            $payload =
                [
                    DetailConstants::IS_ELIGIBLE => $isFeeBasedGatingEligible,
                ];

            // response will be a boolean value - success -> true or false

            $response = $this->pgosProxyController->handlePGOSProxyRequests('merchant_gating_logic_save',
                                                                            $payload, $merchant, true);

            $this->trace->info(TraceCode::PGOS_SAVE_GATING_LOGIC_RESPONSE, [
                'merchant_id' => $merchantId,
                'response'    => $response,
            ]);
        ...
        {
            $this->trace->error(TraceCode::PGOS_SAVE_GATING_LOGIC_FAILURE, [
                'merchant_id'   => $merchantId,
                'error_message' => $exception->getMessage()
            ]);
        }
    ...
    public function fetchMerchantGatingDetails(Merchant\Entity $merchant)
    {
        ...
        try
        {
            ...
            $response = $this->pgosProxyController->handlePGOSProxyRequests('merchant_fetch_gating_logic',
                                                                            $payload, $merchant, true);
    ...
    public function preProcessCreateOrderRequest(array &$body)
    {
        ...
        unset($body['createOrder']);

        $body['create_order'] = $createOrder;

        $this->trace->info(TraceCode::ONBOARDING_PAYMENT_ORDER_CREATE_REQUEST, [
    ...
    private function changeSignupCampaign($signupCampaign, $merchant)
    {
        ...
        $this->app['trace']->info(TraceCode::CHANGE_SIGNUP_CAMPAIGN, [
            'merchant_id'             => $merchant->getId(),
            'current_signup_campaign' => $deviceDetail->getSignupCampaign(),
    ...
    public function constructEventDataForCMMACase($merchant, $eventType): array
    {
        ...
        $kafkaActivationFormSubmissionEventData = [
            DifferEntity::ENTITY_ID                  => $merchant->getId(),
            DifferEntity::ENTITY_NAME                => Constants::MERCHANT,
            DEConstants::EVENT_TYPE                  => $eventType,
            DEConstants::PROCESS_TIMESTAMP           => $processTimestamp,
        ];
