...
/**
 * used for verifying rules and rule_execution_result for ShopEstablishment
 * Any Future verification of rule results can be done in here.
 *
 * Class ShopEstablishmentRuleResultVerifier
 * @package RZP\Models\Merchant\AutoKyc\Bvs\RuleResultVerifier
 */
class ShopEstablishmentRuleResultVerifier extends BaseRuleResultVerifier
{
    ...
    public function verifyAndReturnRuleResultForMatched(): array
    {
        ...
        if ($matchPercentageForRule0 < self::SIGNATORY_NAME_MATCH_THRESHOLD)
        {
            $data[Constants::IS_SIGNATORY_NAME_MATCHED] = false;
        }

        if ($matchPercentageForRule1 < self::COMPANY_NAME_MATCH_THRESHOLD)
        {
