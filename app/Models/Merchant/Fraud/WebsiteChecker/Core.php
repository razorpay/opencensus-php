...
class Core extends Base\Core
{
    ...
    {
        $this->trace->info(TraceCode::WEBSITE_CHECKER_PERIODIC_CRON_STARTED);
    ...
    {
        $this->trace->info(TraceCode::WEBSITE_CHECKER_REMINDER_CRON_STARTED);
        ...
        foreach ($reminderMerchantIdList as $merchantId)
        {
            ...
            $this->notifyRiskChecker($merchantId, Constants::SEND_REMINDER_TO_MERCHANT_JOB, []);
        }
    ...
    private function getMerchantList(int $waitSeconds, string $redisMap): array
    {
        ...
        $this->trace->debug(TraceCode::WEBSITE_CHECKER_DEBUG, [
            'redis_map'                   => $redisMap,
    ...
    {
        $this->trace->info(
            TraceCode::WEBSITE_CHECKER_JOB_ENQUEUE_INITIATED, ['merchant_id' => $merchantId]);
        ...
        try
        {
            ...
            RiskWebsiteChecker::dispatch($this->mode, $jobRequest);
        }
        catch(\Throwable $ex)
        {
            $this->trace->traceException(
                $ex,
                Trace::ERROR,
                TraceCode::WEBSITE_CHECKER_JOB_ENQUEUE_FAILED,
                [
                    'merchant_id' => $merchantId,
                ]
            );
