...
class FriendBuyClient
{
    ...
    public function init($token = null)
    {
        ...
        {
            $headers[RequestHeader::AUTHORIZATION] = $token->getAuthorizationHeader();
        }
    ...
    public function getAuthorization(AuthorizationRequest $request): ?AuthToken
    {
        ...
        try
        {
            $this->trace->info(TraceCode::FRIEND_BUY_SERVICE_REQUEST, [
                Constants::PAYLOAD => $request,
                Constants::PATH    => $request->getPath()
            ]);

            $response = $this->requestAndGetParsedBody($request->getPath(), (array) $request);
    ...
    public function postEvent(FriendBuyRequest $request): ?EventResponse
    {
        $this->init($request->authToken);
        ...
        try
        {
            ...
            $response = $this->requestAndGetParsedBody($request->getPath(), $request->toArray());
    ...
    public function generateReferralLink(ReferralLinkRequest $request): ?ReferralLinkResponse
    {
        ...
        catch (\Throwable $e)
        {
            ...
            return new ReferralLinkResponse([Constants::ERROR, $e->getCode(), Constants::MESSAGE, $e->getMessage()]);
        }
    }
    ...
    public function requestAndGetParsedBody(string $path, array $payload): array
    {
        ...
        {
            $this->trace->info(TraceCode::FRIEND_BUY_SERVICE_RESPONSE, [
    ...
    public function request(string $path, array $payload, int $timeoutMs = null)
    {
        ...
        while ($maxAttempts--)
        {
            try
            {
                $this->trace->info(TraceCode::FRIEND_BUY_SERVICE_REQUEST,$payload);
