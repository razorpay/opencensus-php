...
class LinkedAccount extends Base
{
    ...
    protected function createOrUpdateAccountForEntry(array & $entry)
    {
        ...
        {
            $input       = Helper::getAccountInput($entry);
            $account     = $this->merchantCore
                                ->createSubMerchant($input, $this->merchant);

            $detailInput = Helper::getAccountDetailInput($entry);
            $response    = $this->merchantDetailCore
                                ->saveMerchantDetails($detailInput, $account);
