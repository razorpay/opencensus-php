...
class Service extends Base\Service
{
    ...
    public function trimBeneficiaryNameAndAccountNumber(Merchant\Entity $merchant)
    {
        $merchantId = $merchant->getId();

        $bankAccounts = $this->repo->bank_account->fetchBankAccountWithMerchantIdAndLimit1000($merchant->getId());
        ...
        while (count($bankAccounts) > 0)
        {
            ...
            foreach ($bankAccounts as $bankAccount)
            {
                ...
                $trimmedBeneficiaryName = trim($beneficiaryName);

                $accountNumber = $bankAccount->getAccountNumber();

                $trimmedAccountNumber = trim(str_replace('\n','', $accountNumber));

                if ((strlen($beneficiaryName) > strlen($trimmedBeneficiaryName)) ||
                    (strlen($accountNumber)   > strlen($trimmedAccountNumber)))
                {
                    ...
                    $this->trace->info(
                        TraceCode::BENEFICIARY_NAME_AND_ACCOUNT_NUMBER_TRIMMED,
                        [
                ...
                $lastBankAccountCreatedAt = $bankAccount->getCreatedAt();
            }

            $bankAccounts = $this->repo->contact->fetchContactWithMerchantIdAndLimit1000(
                $merchantId,
                $lastBankAccountCreatedAt
            );
        }

        $this->trace->info(
            TraceCode::BENEFICIARY_NAME_AND_ACCOUNT_NUMBER_TRIMMED_FOR_MERCHANT,
            [
