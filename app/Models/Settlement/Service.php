...
class Service extends Base\Service
{
    ...
    public function postIrctcTransfer()
    {
        $input = Request::all();
        ...
        {
            // Get the amount for captured payments on gateway for last day
            $paymentAmount = $this->repo->payment->getCapturedAmountByMerchant($gateway, $from, $to);

            // Get the amount for refunds on gateway for last day
            $refundAmount = $this->repo->refund->getRefundedAmountByMerchant($gateway, $from, $to);
        ...
        if ($amount > 20000000)
        {
            ...
            $response = (new Kotak\NodalAccount)->generateTransferFile($amount);
        }
        else
        {
            $response = [
