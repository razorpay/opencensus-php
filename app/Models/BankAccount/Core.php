...
//    }
     ...
     */
    public function trimBeneficiaryName($merchantIds, PaginationEntity $paginationEntity)
    {
        $this->trace->info(
        ...
        $bankAccounts = $this->repo->bank_account->fetchBankAccountsHavingSpaceInBeneficiaryName(
            $merchantIds,
        ...
        $bankAccountIds = $bankAccounts->getIds();

        while (count($bankAccounts) > 0)
        {
            ...
            $newBankAccounts = $this->repo->bank_account->fetchBankAccountsHavingSpaceInBeneficiaryName(
                $merchantIds,
                $paginationEntity->getCurrentStartTime(),
                $paginationEntity->getCurrentEndTime(),
                $paginationEntity->getLimit()
            ...
            $newBankAccountIds = $newBankAccounts->getIds();

            $nonCommonIdsFromLastBankAccounts = array_diff($newBankAccountIds, $bankAccountIds);

            if ((count($newBankAccounts) === 0) or
                (count($nonCommonIdsFromLastBankAccounts) > 0))
            {
                $bankAccountIds = $newBankAccountIds;
     ...
     * @param PaginationEntity $paginationEntity
     */
    public function trimAccountNumber($merchantIds, PaginationEntity $paginationEntity)
    {
        $this->trace->info(
        ...
        $bankAccounts = $this->repo->bank_account->fetchBankAccountsHavingSpaceInAccountNumber(
            $merchantIds,
            $paginationEntity->getCurrentStartTime(),
            $paginationEntity->getCurrentEndTime(),
            $paginationEntity->getLimit()
        ...
        while (count($bankAccounts) > 0)
        {
            ...
            $newBankAccounts = $this->repo->bank_account->fetchBankAccountsHavingSpaceInAccountNumber(
                $merchantIds,
                $paginationEntity->getCurrentStartTime(),
                $paginationEntity->getCurrentEndTime(),
                $paginationEntity->getLimit()
