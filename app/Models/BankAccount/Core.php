...
//          TODO: @kartik.sayani - should this be changed?
    ...
    public function buildBankAccountArrayFromMerchantDetail(Merchant\Entity $merchant, bool $linkedAccount = false): array
    {
        $details = $merchant->merchantDetail->toArray();
        ...
        $data = [
            ...
            Entity::BENEFICIARY_COUNTRY   => $merchant->getCountry(),
            Entity::BENEFICIARY_MOBILE    => $details[DetailEntity::CONTACT_MOBILE],
        ];

        if(Country::matches($merchant->getCountry() , Country::IN))
        {
            $data[Entity::BANK_IDENTIFIER] = $details[DetailEntity::BANK_BRANCH_IFSC];
            $data[Entity::IDENTIFIER_TYPE] = Detail\BankBranchCodeType::IFSC;
        }else{
            $data[Entity::BANK_IDENTIFIER]       = $details[DetailEntity::BANK_BRANCH_CODE];
            $data[Entity::IDENTIFIER_TYPE]       = $details[DetailEntity::BANK_BRANCH_CODE_TYPE];
        }
