...
trait Authorize
{
    ...
    protected function authorizeAcrossTerminals(Payment\Entity $payment, array $input, array & $gatewayInput)
    {
        ...
        while ($retryAttempts < $maxRetryAttempts)
        {
            ...
            try
            {
                ...
                else if ($payment->isCollectXPayment() === true)
                {
    ...
    protected function isCollectxPayment($payment): bool
    {
        if ($this->merchant->isFeatureEnabled(Features::COLLECTX_ENABLED) === true &&
            $payment[Payment\Entity::REFERENCE14] === Constant::COLLECTX)
        {
            return true;
        }
        return false;
    ...
    protected function validateTokenExpiredAt(Token\Entity $token, Payment\Entity $payment)
    {
        ...
        if((($token->getMethod() === Method::EMANDATE) or
                (($token->getMethod() === Method::NACH) and ($currentTime >= 1698777000)) or
                ($token->getMethod() === Method::UPI)) and
            ($payment->isRecurringTypeInitial() === true) and
