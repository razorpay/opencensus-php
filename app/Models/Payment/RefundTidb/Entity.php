...
use App;
use Carbon\Carbon;
use RZP\Models\Payment\Refund\Core;
...
class Entity extends Base\PublicEntity
{
    ...
    public function processArrayPublicAndReturn(array $response)
    {
        ...
        if ($exposeExtraAttributes === true)
        {
            ...
            $response[Constants::REFUND_TYPE] = (new Core)->getRefundType($this->getId(), $this->merchant, $this->getBatchId(), $this->isScrooge());
        }
...
//    }
    ...
    public function wasGatewayRefundNotSupportedAtCreation($refundId): bool
    {
        ...
        if ((isset($scroogeRefund[Constants::RESPONSE_BODY]) === true) and
            (isset($scroogeRefund[Constants::RESPONSE_BODY][Constants::META]) === true) and
            (isset($scroogeRefund[Constants::RESPONSE_BODY][Constants::META][Constants::PAYMENT_AGE_LIMIT_FOR_GATEWAY_REFUND]) === true))
        {
            return true;
        }
    ...
    private function getMessageForTransactionTracker(TransactionTrackerMessages $transactionTrackerMessages, int $days, Carbon $expectedDate, $messageType): string
    {
        ...
        {
            $messageStatus = Constants::FAILED_AGED;
