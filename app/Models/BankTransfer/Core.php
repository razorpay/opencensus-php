...
class Core extends Base\Core
{
     ...
     * @return Entity
     */
    public function reassign(Entity $bankTransfer, array $input)
    {
        $targetMerchant = $this->repo
                               ->merchant
                               ->findOrFailPublic($input[Entity::MERCHANT_ID]);

        $bankTransfer->getValidator()->validateReassignment();

        $targetVirtualAccount = $this->getTargetVirtualAccount($bankTransfer, $input['provider']);

        $bankTransfer->getValidator()->validateReassignmentTarget($targetMerchant, $targetVirtualAccount);
        ...
        {
            $this->updateOriginalVirtualAccount($bankTransfer);

            $this->updatePayerBankAccount($bankTransfer, $targetMerchant, $targetVirtualAccount);

            $this->updateBankTransfer($bankTransfer, $targetMerchant, $targetVirtualAccount);

            $this->updatePayment($bankTransfer, $targetMerchant);

            $this->updateVirtualAccount($bankTransfer, $targetVirtualAccount);
        });

        $paymentProcessor = new PaymentProcessor($targetMerchant);

        $paymentProcessor->autoCapturePayment($bankTransfer->payment);
    ...
    protected function updatePayerBankAccount(
        Entity $bankTransfer,
        ...
        VirtualAccount\Entity $targetVirtualAccount)
    {
        ...
        $payerBankAccount->associateVirtualAccount($targetVirtualAccount);

        $this->repo->saveOrFail($payerBankAccount);
    }

    protected function updateBankTransfer(
        Entity $bankTransfer,
        Merchant\Entity $targetMerchant,
        VirtualAccount\Entity $targetVirtualAccount)
    {
        $bankTransfer->merchant()->associate($targetMerchant);

        $bankTransfer->virtualAccount()->associate($targetVirtualAccount);
        ...
        Merchant\Entity $targetMerchant)
    {
        ...
        $payment->merchant()->associate($targetMerchant);

        $this->repo->saveOrFail($payment);
    }

    protected function updateVirtualAccount(
        Entity $bankTransfer,
        VirtualAccount\Entity $targetVirtualAccount)
    {
        $targetVirtualAccount->updateWithBankTransfer($bankTransfer);

        $this->repo->saveOrFail($targetVirtualAccount);
    }

    protected function getTargetVirtualAccount(Entity $bankTransfer, string $provider)
    {
        VirtualAccount\Provider::validateLiveProvider($provider);

        $bankCode = VirtualAccount\Provider::getBankCode($provider);
