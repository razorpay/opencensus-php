...
class Repository extends Base\Repository
{
    ...
    public function findByCaseInsensitiveUtrAndPayeeAccountAndAmount(string $utr, $payeeAccount, int $amount)
    {
        $findByUtr = $this->findByUtrAndPayeeAccountAndAmount($utr, $payeeAccount, $amount);
        if ($findByUtr !== null) {
            return $findByUtr;
        }
        ...
        $upperCaseUTRQuery = $this->newQuery()
            ->where(Entity::UTR, '=', $utr)
            ->where(Entity::PAYEE_ACCOUNT, '=', $payeeAccount)
            ->where(Entity::AMOUNT, '=', $amount);

        if ($upperCaseUTRQuery->first() !== null) {
            return $upperCaseUTRQuery->first();
        } else {
            $utr = strtolower($utr);
            $lowerCaseUTRQuery = $this->newQuery()
                ->where(Entity::UTR, '=', $utr)
                ->where(Entity::PAYEE_ACCOUNT, '=', $payeeAccount)
                ->where(Entity::AMOUNT, '=', $amount);

            return $lowerCaseUTRQuery->first();
        }
