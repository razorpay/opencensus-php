...
final class AESGCM
{
    ...
    {
        self::assertString($K, 'The key encryption key must be a binary string.');
        $key_length = mb_strlen($K, '8bit') * 8;
        self::assertInArray($key_length, [128, 192, 256], 'Bad key encryption key length.');
        self::assertString($IV, 'The Initialization Vector must be a binary string.');
        self::assertNullOrString($P, 'The data to encrypt must be null or a binary string.');
        self::assertNullOrString($A, 'The Additional Authentication Data must be null or a binary string.');
        self::assertInteger($tag_length, 'Invalid tag length. Supported values are: 128, 120, 112, 104 and 96.');
        self::assertInArray($tag_length, [128, 120, 112, 104, 96], 'Invalid tag length. Supported values are: 128, 120, 112, 104 and 96.');
    ...
    private static function encryptWithPHP71($K, $key_length, $IV, $P = null, $A = null, $tag_length = 128)
    {
        ...
        self::assertTrue(false !== $C, 'Unable to encrypt the data.');
    ...
    public static function decrypt($K, $IV, $C, $A, $T)
    {
        ...
        self::assertInArray($key_length, [128, 192, 256], 'Bad key encryption key length.');
        self::assertString($IV, 'The Initialization Vector must be a binary string.');
        self::assertNullOrString($C, 'The data to encrypt must be null or a binary string.');
        self::assertNullOrString($A, 'The Additional Authentication Data must be null or a binary string.');
    ...
    private static function decryptWithPHP71($K, $key_length, $IV, $C, $A, $T)
    {
        ...
        self::assertTrue(false !== $P, 'Unable to decrypt or to verify the tag.');
    ...
    private static function decryptWithPHP($K, $key_length, $IV, $C, $A, $T, $tag_length = 128)
    {
        ...
        self::assertEq($T1, $T, 'Unable to decrypt or to verify the tag.');
    ...
    private static function common($K, $key_length, $IV, $A)
    {
        ...
        if (96 === $iv_len) {
            ...
            self::assertEq(($s + 64) % 8, 0, 'Unable to decrypt or to verify the tag.');
    ...
    public static function assertString($value, string $message = 'Value not a string')
    {
    ...
    public static function assertInArray($value, array $array, string $message = 'Value not found in array')
    {
    ...
    public static function assertNullOrString($value, string $message = 'Value neither a string nor null')
    {
        if (is_null($value) === false)
        {
            self::assertString($value, $message);
        }
    }

    public static function assertInteger($value, string $message = 'Value not an integer')
    {
