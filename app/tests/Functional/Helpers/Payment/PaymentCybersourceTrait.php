...
trait PaymentCybersourceTrait
{
    ...
    {
        $tds = $this->is3d($response, $callback);

        $content = $response->getContent();

        if ($tds)
        {
            return $this->run3dSecure($response, $callback);
        }

        return $response;
    }

    protected function run3dSecure($response, &$callback = null)
    {
    ...
    protected function is3d($response, $callback = null)
    {
        $content = $response->getContent();
