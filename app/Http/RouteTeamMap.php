...
class RouteTeamMap
{
     ...
     */
    protected static $teamSlackTagMap = [
        self::TEAM_PAYMENTS_DASHBOARD       => '<!subteam^S042X0V59MZ>',
        self::TEAM_PAYMENTS_GROWTH          => '<!subteam^S03QFF05PA5>',
        self::TEAM_PAYMENTS_RISK            => '<!subteam^SQ15LQNQL>',
    ...
    /**
     * Downstream API route mapping
     *
     * @var array[]
     */
    protected static $apiRouteTeamMap = [
        ...
        'setl_get_creation_status'                        => [self::TEAM_PAYMENTS_SETTLEMENTS],
        'setl_update_creation_status'                     => [self::TEAM_PAYMENTS_SETTLEMENTS],
        'setl_execution_release'                          => [self::TEAM_PAYMENTS_SETTLEMENTS],
    ...
    {
        $route          = $route ?? '';
        $apiRoute       = array_get(self::$apiRouteTeamMap, $route, []);
        $dashboardRoute = array_get(self::$routeTeamMap, $route, []);

        if (empty($dashboardRoute) === true && empty($apiRoute) === true)
        {
            return self::TEAM_UNKNOWN;
        }

        if (empty($dashboardRoute) === false)
        {
            return implode(',', self::$routeTeamMap[$route]);
        }

        return implode(',', self::$apiRouteTeamMap[$route]);
    }

    /**
     * @param string|null $teamName could be comma separated as well
     *
     * @return string
     */
    public static function getTeamSlackTag(?string $teamName): string
    {
        if (empty($teamName) === true)
        {
            return self::getTeamSlackTag(self::TEAM_DEFAULT);
        }
        ...
        {
            $tag = array_get(self::$teamSlackTagMap, $team);

            if (empty($tag) === true)
            {
                $tag = self::$teamSlackTagMap[self::TEAM_DEFAULT];
            }
