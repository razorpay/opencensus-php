...
class Reconciliate extends Base\Core
{
    ...
    //before sending requests to scrooge api, we need to break them into chunks of <= this size
    ...
    protected function getRefundIdFromScrooge(array $input, $gateway, $referenceKey)
    {
        ...
        foreach (array_chunk($input, self::SCROOGE_CHUNK_SIZE) as $chunks)
        {
            try {
                ...
                $responses[] = $this->app['scrooge']->getRefundsFromPaymentIdAndGatewayId($request);
            }
            catch (\Exception $e)
            {
