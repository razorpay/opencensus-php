...
class GatewayTraceProcessor extends ApiTraceProcessor
{
    protected $input;

    protected $action;

    public function __construct($app, array $input, $action)
    {
    ...
    {
        if (isset($record['request']['merchant_id']) === false)
        {
    ...
    {
        if (isset($record['context']['action']) === false)
        {
