...
class TransferUpdate extends Job
{
    ...
    protected function traceFailureMetrics(string $functionName, $startTime)
    {
        ...
        $trace->count(Metric::EXTERNAL_TRANSFER_REPO_SAVE_FAILURE, [
            'caller'      => $functionName,
        ]);

        $trace->histogram(Metric::EXTERNAL_TRANSFER_REPO_SAVE_FAILURE_TIME_TAKEN,
            millitime() - $startTime,
            [
                'caller'  => $functionName
            ]);
    ...
    protected function traceSuccessMetrics(string $functionName, $startTime)
    {
        ...
        $trace->count(Metric::EXTERNAL_TRANSFER_REPO_SAVE_SUCCESS, [
            'caller'      => $functionName,

        ]);

        $trace->histogram(Metric::EXTERNAL_TRANSFER_REPO_SAVE_SUCCESS_TIME_TAKEN,
            millitime() - $startTime,
            [
                'caller'  => $functionName
            ]);
