...
class RxFundAccountDualWrite extends Job
{
     ...
     * @var string
     */
    protected $queueConfigKey = 'rx_fund_account_dual_write';
    ...
    public function handle()
    {
        try
        {
            ...
            $this->trace->info(
                TraceCode::RX_FUND_ACCOUNT_DUAL_WRITE_JOB_INIT,
                $this->params
            );

            (new FundAccount\Core)->processRxDualWrite($this->params);

            $this->trace->info(
                TraceCode::RX_FUND_ACCOUNT_DUAL_WRITE_JOB_COMPLETE,
                $this->params
            );
        ...
        catch (\Throwable $exception)
        {
            $this->trace->traceException(
                $exception,
                Trace::ERROR,
                TraceCode::RX_FUND_ACCOUNT_DUAL_WRITE_FAILURE,
                $this->params);
    ...
    protected function checkRetry()
    {
        ...
        {
            $this->trace->info(
                TraceCode::RX_FUND_ACCOUNT_DUAL_WRITE_JOB_RELEASE,
                $this->params);
        ...
        {
            $this->trace->info(
                TraceCode::RX_FUND_ACCOUNT_DUAL_WRITE_JOB_DELETE,
                $this->params);
