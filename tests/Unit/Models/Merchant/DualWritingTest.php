...
class DualWritingTest extends TestCase
{
    ...
    protected function createAndFetchMocks($mid)
    {
        Mail::fake();

        $merchant = $this->fixtures->create('merchant', ['id' => $mid]);

        $merchantDetail = $this->fixtures->create('merchant_detail', ['merchant_id' => $mid]);

        $splitzMockInput = [
            'id'            => $mid,
            'experiment_id' => \Config::get('app.pgos_shadow_mode_experiment_id')
        ];
    ...
    public function testSaveOnboardingDataToAPI()
    {
        ...
        $merchant1 = (new \RZP\Models\Merchant\Detail\Repository)->find('KqsQEszAud2PqZ');
    ...
    {
        $this->createAndFetchMocks('LKDtR1ECoLNx5g');
        ...
        $merchantDetail1 = (new \RZP\Models\Merchant\Detail\Repository)->find('LKDtR1ECoLNx5g');
