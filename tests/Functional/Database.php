...
class Database
{
    ...
    protected function createDatabases()
    {
        ...
        $this->db->connection('mysql_init')->getPdo()->exec("GRANT ALL PRIVILEGES ON `{$apiLiveDb}`.* TO 'root'@'localhost'");

        $apiTestDb = env('DB_TEST_DATABASE', 'api_test');
        $this->db->connection('mysql_init')->getPdo()->exec("CREATE DATABASE IF NOT EXISTS `{$apiTestDb}`");
        $this->db->connection('mysql_init')->getPdo()->exec("GRANT ALL PRIVILEGES ON `{$apiTestDb}`.* TO 'root'@'localhost'");
