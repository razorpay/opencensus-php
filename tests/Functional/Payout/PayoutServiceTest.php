...
//            )
    ...
    public function mockPayoutServiceFreePayoutSet($fail = false, $currentAccount = false, $request = [])
    {
        // Not mocking this method like mockPayoutServiceStatus because we need to assert for the request headers that
        // are going to be sent to payout service.
        ...
        $freePayoutSetMock->shouldReceive('sendRequest')
            ...
            // response based on $request, if needed, that can also be added here using
            // andReturnUsing method instead of andReturn
                $this->freePayoutSetResponseForPayoutServiceMock($fail, $currentAccount)
            );

        $this->app->instance(PayoutServiceFreePayout::PAYOUT_SERVICE_FREE_PAYOUT, $freePayoutSetMock);
    }

    public function freePayoutSetResponseForPayoutServiceMock($fail, $currentAccount)
    {
        $response = new \WpOrg\Requests\Response();
    ...
    public function mockPayoutServiceGetFreePayout($fail = false, $currentAccount = false, $request = [])
    {
        // Not mocking this method like mockPayoutServiceStatus because we need to assert for the request headers that
        // are going to be sent to payout service.
        $payoutServiceGetMock = Mockery::mock('RZP\Services\PayoutService\Get',
        ...
        $payoutServiceGetMock->shouldReceive('sendRequest')
                             ...
                             // response based on $request, if needed, that can also be added here using
                             // andReturnUsing method instead of andReturn
                                 $this->freePayoutGetResponseForPayoutServiceMock($fail, $currentAccount)
                             );

        $this->app->instance(PayoutServiceGet::PAYOUT_SERVICE_GET, $payoutServiceGetMock);
    }

    public function freePayoutGetResponseForPayoutServiceMock($fail, $currentAccount)
    {
        $response = new \WpOrg\Requests\Response();
        ...
        else if ($currentAccount)
        {
            $response->body        = json_encode(
                [
                    'free_payouts_count'           => FreePayout::DEFAULT_FREE_DIRECT_ACCOUNT_PAYOUTS_COUNT_RBL_SLAB1,
   ...
   //check create payment from payouts service for axis cc
    ...
    public function testCreatePayoutViaMicroserviceAndPassFundAccountInfo()
    {
        ...
        $expectedFundAccountExtraInfo = [
            ...
            'bank_account' => [
                ...
                'bank_identifier' => $bankAccountObject['bank_identifier'],
                'ifsc_code' => $bankAccountObject['ifsc_code'],
                'account_type' => $bankAccountObject['account_type'],
                'beneficiary_name' => $bankAccountObject['beneficiary_name'],
                'beneficiary_city' => $bankAccountObject['beneficiary_city'],
                'beneficiary_email' => $bankAccountObject['beneficiary_email'],
                'beneficiary_state' => $bankAccountObject['beneficiary_state'],
                'beneficiary_mobile' => $bankAccountObject['beneficiary_mobile'],
                'beneficiary_address1' => $bankAccountObject['beneficiary_address1'],
                'beneficiary_country' => $bankAccountObject['beneficiary_country'],
                'identifier_type' => $bankAccountObject['identifier_type'],
                'virtual' => $bankAccountObject['virtual'],
            ],
            'contact' => [
    ...
    public function testCreatePayoutForBankAccountViaMicroserviceAndPreCalculatePricingInfoToUpdatePayoutServiceRedis()
    {
        ...
        $expectedFundAccountExtraInfo = [
            ...
            'bank_account' => [
                ...
                'account_number' => $bankAccountObject['account_number'],
                'bank_name'      => $bankAccountObject['bank_name'],
                'ifsc_code' => $bankAccountObject['ifsc'],
                'account_type' => $bankAccountObject['account_type'],
                'beneficiary_name' => $bankAccountObject['beneficiary_name'],
                'beneficiary_city' => $bankAccountObject['beneficiary_city'],
                'beneficiary_email' => $bankAccountObject['beneficiary_email'],
    ...
    public function testFreePayoutMigrationAdminActionForMerchantWithOnboardingTime()
    {
        $this->mockPayoutServiceFreePayoutMigration();
        ...
        $bankingAccount = $this->getDbEntities('banking_accounts',
            [
                'account_number'   => '2224440041626905',
            ], 'live')->first();


        $psBankingAccountData = [
            'id'          => 'randomid111123',
            'merchant_id'   => $this->bankingBalance->getMerchantId(),
            'balance_id'   => $this->bankingBalance->getId(),
            'channel' => $this->bankingBalance->getChannel(),
            ...
            'account_type' => 'direct',
            'fts_fund_account_id' =>'random',
            'payout_service_enabled' => 0,
            'created_at'  => Carbon::now(Timezone::IST)->getTimestamp(),
            'updated_at'  => Carbon::now(Timezone::IST)->getTimestamp(),
        ];

        \DB::connection('test')->table('ps_banking_accounts')->insert($psBankingAccountData);
        ...
        $this->assertEquals($this->bankingBalance->getCreatedAt(), $psBankingAccount['created_at']);
        $this->assertEquals( 1, $psBankingAccount['payout_service_enabled']);
        $this->assertContains(Feature\Constants::LEDGER_REVERSE_SHADOW, $liveFeaturesArray);
        $this->assertContains(Feature\Constants::PAYOUT_SERVICE_ENABLED, $liveFeaturesArray);
        $this->assertContains(Constants::IDEMPOTENCY_API_TO_PS, $liveFeaturesArray);
        $this->assertNotContains(Constants::IDEMPOTENCY_PS_TO_API, $liveFeaturesArray);
    ...
    {
        $this->mockPayoutServiceFreePayoutSet(false, true);
        ...
        $this->testData[__FUNCTION__]['request']['free_payouts_count'] = 122;

        $this->testData[__FUNCTION__]['request']['url'] =
            '/balance/' . $balance[Balance::ID] . '/free_payout';

        $this->testData[__FUNCTION__]['response']['content']['free_payouts_count'] = 122;
    ...
    {
        $this->mockPayoutServiceGetFreePayout(false, true);
        ...
        $this->testData[__FUNCTION__]['response']['content']['free_payouts_count'] =
            FreePayout::DEFAULT_FREE_DIRECT_ACCOUNT_PAYOUTS_COUNT_RBL_SLAB1;
