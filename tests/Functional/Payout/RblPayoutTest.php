...
class RblPayoutTest extends TestCase
{
    ...
    public function testMinimumBalanceCheckForQueuedPayoutDispatch()
    {
        $oldDateTime = Carbon::create(2020, 01, 21, 12, 23, null, Timezone::IST);

        $ba = $this->fixtures->edit('banking_account', 'xba00000000000', [
            'balance_last_fetched_at' => $oldDateTime->getTimestamp(),
        ]);
        ...
        (new Admin\Service)->setConfigKeys([Admin\ConfigKey::MINIMUM_BALANCE_QUEUING_THRESHOLD => [$ba->getBalanceId() => 100]]);
//        (new Admin\Service)->setConfigKeys([Admin\ConfigKey::MINIMUM_BALANCE_QUEUING_THRESHOLD => [$ba->getBalanceId()."1" => 100]]);

        $this->mockMozartResponseForFetchingBalanceFromRblGateway(4000);
        ...
        // Add enough balance to allow the payout to get processed
        $this->mockMozartResponseForFetchingBalanceFromRblGateway(4001);
