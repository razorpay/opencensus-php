...
class IciciBankingAccountStatementTest extends TestCase
{
    ...
    // asserting external credit and payout events to ledger
    public function testUtrMappingForIMPSWithLedgerReverseShadow()
    {
        ...
        $this->assertEquals(1, count($externalEntries));
        $this->assertNull($payoutTxn);

        $this->assertNull($payout['transaction_id']);
        $this->assertNull($externalEntries[0]['transaction_id']);
    ...
    public function testUtrMappingForFeePayoutWithLedgerReverseShadow()
    {
        $this->fixtures->merchant->addFeatures([Features::DA_LEDGER_REVERSE_SHADOW]);
    ...
    // asserting external credit, external debit, payout and reversal events to ledger
    public function testWebhookEventForIciciAccountStatementForSuccessfulMappingToReversalWithLedgerReverseShadow()
    {
        $this->fixtures->merchant->addFeatures([Features::DA_LEDGER_REVERSE_SHADOW]);
        ...
        // Note: In this test, we don't find any existing reversal, so as per the flow we look for existing payout for this credit row and find one
        // But we are not able to do re-linking this payout (from corresponding external) as the test has reverse shadow feature on merchant
        // and thus the BAS rows are not linked to any transaction yet, so bas is not found for debit mapping BAS_NOT_FOUND_FOR_DEBIT_MAPPING
        $transactorTypeArray = [
            'da_ext_credit',
            'da_ext_debit',
            'da_payout_reversed',
    ...
    public function testWebhookEventForIciciAccountStatementForSuccessfulMappingToReversalForFeePayoutWithLedgerReverseShadow()
    {
        $this->fixtures->merchant->addFeatures([Features::DA_LEDGER_REVERSE_SHADOW]);
