...
class MerchantWrapperTest extends TestCase
{
    ...
    function testMerchantWrapperFindOrFail()
    {
        ...
        $traceMock = $this->createTraceMock();
        $traceMock->expects($this->never())->method('traceException');
        $traceMock->expects($this->exactly(1))->method('info')->with(TraceCode::ASV_COMPARE_MISMATCH, ["entity" => "merchant", "id" => $merchantId, "difference" => ["org_id"]]);
        $traceMock->expects($this->exactly(1))->method('count')->with(Metric::ASV_COMPARE_MISMATCH, ["merchant"]);
        $accountAsvClientMock = $this->createAccountAsvClientMock();
...
//            withConsecutive([$merchantId, CONSTANT::SHADOW, CONSTANT::READ], [$merchantId, CONSTANT::REVERSE_SHADOW, CONSTANT::READ])->willReturnOnConsecutiveCalls(false, true);
        ...
        $traceMock->expects($this->exactly(1))->method('traceException')->with($exception, Trace::ERROR, TraceCode::ASV_READ_SHADOW_EXCEPTION, ["id" => $merchantId, "entity" => "merchant"]);
        ...
        $traceMock = $this->createTraceMock();
        $traceMock->expects($this->exactly(1))->method('traceException')->with($exception, Trace::CRITICAL, TraceCode::ASV_REVERSE_SHADOW_EXCEPTION, ["id" => $merchantId, "entity" => "merchant"]);
