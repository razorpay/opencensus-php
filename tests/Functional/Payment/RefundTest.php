...
//        $this->mockRefundEmail();
    ...
    /**
     * Tests if all the late authorized payments of invoice which is in expired
     * state are refunded properly via cron api.
     *
     * @return null
     */
    public function testRefundOfLateAuthPaymentsOfExpiredInvoices()
    {
        //
        // - Creates multiple invoices with one or more late authorized payments
        //   (as well as normal payments), some of the invoices are expired.
        // - Hits cron api
        // - Verifies response
        // - Verifies entities
        //

        $expectedRefundedPaymentIds = [];

        $orders = $this->fixtures->times(4)->create('order');

        $orderId0 = $orders[0]->getId();
        ...
        // Both of following payments should get refunded
        $payments = $this->fixtures->times(2)->create('payment:authorized',
            [
                'order_id'        => $orderId0,
        ...
        // Following shouldn't get refunded too
        $this->fixtures->create('payment:authorized',
            [
                'order_id'        => $orderId1,
                'invoice_id'      => '1000001invoice',
                'late_authorized' => 1,
            ]);

        $orderId2 = $orders[2]->getId();
