...
class NbplusNetbankingCbiReconciliationTest extends NbPlusPaymentServiceNetbankingTest
{
    use ReconTrait;
    ...
    public function testCbiSuccessRecon()
    {
        ...
        $this->reconcile($uploadedFile, Base::NETBANKING_CBI);
    ...
    public function testCbiSuccessReconWithForceAuthorize()
    {
        ...
        $data = $this->testData['testCbiSuccessRecon'];
        ...
        $this->reconcile($uploadedFile, Base::NETBANKING_CBI, [$payment['public_id']]);
    ...
    {
        $formattedData = ''.$data['payment_id'].'^'.$data['bank ref no.'].'^500^Y^'.$data['date'].'^HS-123456789^01';
