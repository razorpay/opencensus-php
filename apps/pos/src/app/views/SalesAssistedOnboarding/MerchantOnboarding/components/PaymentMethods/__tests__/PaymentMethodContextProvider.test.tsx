...
describe('<PaymentMethods/>', () => {
  ...
  test('should be able to go back from select onboarding model', async () => {
    server.use(
      getModularConfig({
        type: 'success',
        data: {
  ...
  test('should be able to select onboarding model and proceed further', async () => {
    server.use(
      getModularConfig({
        type: 'success',
        data: {
    ...
    const directModelRadioButton = screen.getByRole('radio', { name: /Direct Model/i });
    await userEvent.click(directModelRadioButton);
    await waitFor(() => expect(directModelRadioButton).toBeChecked());
...
describe('<PaymentMethodForm/>', () => {
  ...
  test('should disable save and continue when editing rates', async () => {
    ...
    const textFile = new File(['sample content'], 'test-file.pdf', {
      type: 'text/plain',
    });
    await userEvent.upload(fileInput, textFile);
    expect(
      screen.queryByText('Some Error Occured while upload file. Please try again later'),
    ).toBeInTheDocument();
    ...
    const ccEmiRateCheckbox = document.querySelector('input[name="vas_cc_emi_rate_field"]');
    expect(ccEmiRateCheckbox).not.toBeNull();
    await userEvent.click(ccEmiRateCheckbox as Element);

    const dcEmiRateCheckbox = document.querySelector('input[name="vas_dc_emi_rate_field"]');
    expect(dcEmiRateCheckbox).not.toBeNull();
    await userEvent.click(dcEmiRateCheckbox as Element);
...
describe('<NachForm/>', () => {
  test('should render nach form component', async () => {
    const props = {
      nachForm: {
    ...
    await userEvent.click(screen.getByRole('button', { name: /skip & add later/i }));
  });

  test('should throw error for invalid NACH file', async () => {
    const props = {
      nachForm: {
        [NachFormKeyNames.NACH_FORM_DOCUMENT_FIELD]: [],
    ...
    await userEvent.upload(fileInput, textFile);
    expect(screen.queryByText('Unable to upload file. Please try again')).toBeInTheDocument();
  });
});
