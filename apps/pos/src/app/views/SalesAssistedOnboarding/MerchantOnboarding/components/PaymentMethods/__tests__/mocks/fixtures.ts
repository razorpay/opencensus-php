import {
  BRAND_EMI_VERIFICATION_STATUS_ENUM,
  ModularOnboardingField,
} from 'apps/pos/src/app/types/modular';
import {
  PaymentMethodFormType,
  PaymentMethodsFieldKeyNames,
} from 'apps/pos/src/app/types/PaymentsAndService';

export const MOCK_BRAND_RELATED_FIELDS = [
  {
    name: 'dealer_code_field',
    isDisabled: false,
    isRequired: false,
    isHidden: false,
    meta: {
      title: 'Dealer Code',
      description: '',
      defaultValue: '',
      size: '',
      accessibilityLabel: '',
      hideOnReviewScreen: false,
      dataType: 'string',
      selectionType: '',
    },
    failureReason: '',
    failureReasonType: '',
    stringValue: '',
  },
  {
    name: 'merchant_gst_field',
    isDisabled: false,
    isRequired: false,
    isHidden: false,
    meta: {
      title: 'Merchant GST',
      description: '',
      defaultValue: '',
      size: '',
      accessibilityLabel: '',
      hideOnReviewScreen: false,
      dataType: 'string',
      selectionType: '',
    },
    failureReason: '',
    failureReasonType: '',
    stringValue: '',
  },
];

export const getMockBrandEmiFields = ({
  storeType = '',
  brandName = '',
  dealerCode = '',
  stateCode = '',
  distributorCode = '',
  verificationStatus = BRAND_EMI_VERIFICATION_STATUS_ENUM.PENDING,
}): ModularOnboardingField[] => [
  {
    name: 'store_type_field',
    isDisabled: false,
    isRequired: true,
    isHidden: false,
    meta: {
      title: 'Type of Store',
      description: 'null',
      defaultValue: 'null',
      size: 'null',
      accessibilityLabel: 'null',
      dataType: 'select',
      selectionType: 'null',
      options: [
        {
          label: 'Multi Brand Outlet',
          value: 'multi_brand_outlet',
        },
        {
          label: 'Exclusive Brand Outlet',
          value: 'exclusive_brand_outlet',
        },
        {
          label: 'Large Format Retail',
          value: 'large_format_retail',
        },
      ],
      validations: [
        {
          condition: 'true',
          errorMessage: 'Store type is required',
          type: 'isRequired',
        },
      ],
    },
    failureReason: '',
    failureReasonType: '',
    stringValue: storeType ?? '',
  },
  {
    name: 'brand_name_field',
    isDisabled: false,
    isRequired: true,
    isHidden: false,
    meta: {
      title: 'Brand Name',
      description: 'null',
      defaultValue: 'null',
      size: 'null',
      accessibilityLabel: 'null',

      dataType: 'select',
      selectionType: 'null',
      options: [
        {
          label: 'Akai',
          value: 'akai',
        },
        {
          label: 'BenQ',
          value: 'benq',
        },
        {
          label: 'Bluestar',
          value: 'bluestar',
        },
        {
          label: 'Bosch',
          value: 'bosch',
        },
        {
          label: 'Daewoo',
          value: 'daewoo',
        },
        {
          label: 'Diasun',
          value: 'diasun',
        },
        {
          label: 'ElanPro',
          value: 'elanpro',
        },
        {
          label: 'Elista',
          value: 'elista',
        },
        {
          label: 'Evra Energy',
          value: 'evra energy',
        },
        {
          label: 'Havells',
          value: 'havells',
        },
        {
          label: 'Itel',
          value: 'itel',
        },
        {
          label: 'Jajot',
          value: 'jajot',
        },
        {
          label: 'Lebu Electric',
          value: 'lebu electric',
        },
        {
          label: 'Leela Eye Institute',
          value: 'leela eye institute',
        },
        {
          label: 'Lloyd',
          value: 'lloyd',
        },
        {
          label: 'Lords Automotive',
          value: 'lords automotive',
        },
        {
          label: 'Nexzu Mobility',
          value: 'nexzu mobility',
        },
        {
          label: 'Nokia',
          value: 'nokia',
        },
        {
          label: 'Oneiric',
          value: 'oneiric',
        },
        {
          label: 'Oppo',
          value: 'oppo',
        },
        {
          label: 'Realme',
          value: 'realme',
        },
        {
          label: 'Samsung',
          value: 'samsung',
        },
        {
          label: 'Sharp',
          value: 'sharp',
        },
        {
          label: 'Siemens',
          value: 'siemens',
        },
        {
          label: 'Slim Nutrifit',
          value: 'slim nutrifit',
        },
        {
          label: 'Tecno',
          value: 'tecno',
        },
        {
          label: 'Tejas',
          value: 'tejas',
        },
        {
          label: 'Vivo',
          value: 'vivo',
        },
        {
          label: 'Voltas',
          value: 'voltas',
        },
        {
          label: 'Voltas BeKo',
          value: 'voltas beko',
        },
        {
          label: 'Vsun',
          value: 'vsun',
        },
        {
          label: 'Xiaomi',
          value: 'xiaomi',
        },
      ],
      validations: [
        {
          condition: 'true',
          errorMessage: 'Brand name is required',
          type: 'isRequired',
        },
      ],
    },
    failureReason: '',
    failureReasonType: '',
    stringValue: brandName ?? '',
  },
  {
    name: 'fetch_brand_data_fields',
    isDisabled: false,
    isRequired: false,
    isHidden: false,
    meta: {
      title: '',
      description: '',
      defaultValue: '',
      size: '',
      accessibilityLabel: '',

      dataType: 'string',
      selectionType: '',
    },
    failureReason: '',
    failureReasonType: '',
    stringValue: 'vivo',
  },
  {
    name: 'dealer_code_field',
    isDisabled: false,
    isRequired: false,
    isHidden: false,
    meta: {
      title: 'Dealer Code',
      description: '',
      defaultValue: '',
      size: '',
      accessibilityLabel: '',

      dataType: 'string',
      selectionType: '',
    },
    failureReason: '',
    failureReasonType: '',
    stringValue: dealerCode,
  },
  {
    name: 'distributor_code_field',
    isDisabled: false,
    isRequired: false,
    isHidden: false,
    meta: {
      title: 'Distributor Code',
      description: '',
      defaultValue: '',
      size: '',
      accessibilityLabel: '',

      dataType: 'string',
      selectionType: '',
    },
    failureReason: '',
    failureReasonType: '',
    stringValue: distributorCode,
  },
  {
    name: 'state_code_field',
    isDisabled: false,
    isRequired: false,
    isHidden: false,
    meta: {
      title: 'State Code',
      description: '',
      defaultValue: '',
      size: '',
      accessibilityLabel: '',

      dataType: 'string',
      selectionType: '',
    },
    failureReason: '',
    failureReasonType: '',
    stringValue: stateCode,
  },
  {
    name: 'merchant_gst_field',
    isDisabled: false,
    isRequired: false,
    isHidden: false,
    meta: {
      title: 'Merchant GST',
      description: '',
      defaultValue: '',
      size: '',
      accessibilityLabel: '',
      dataType: 'string',
      selectionType: '',
    },
    failureReason: '',
    failureReasonType: '',
    stringValue: '',
  },
  {
    name: 'brand_details_summary',
    isDisabled: false,
    isRequired: true,
    isHidden: false,
    meta: {
      title: 'Brand Details Summary',
      description: 'null',
      defaultValue: 'null',
      size: 'null',
      accessibilityLabel: 'null',
      dataType: 'BrandDetailsSummary',
      selectionType: 'null',
    },
    failureReason: '',
    failureReasonType: '',
    addedBrands: [
      {
        name: 'xiaomi',
        dealerCode: 'KOPLQ6789',
        distributorCode: '',
        stateCode: '',
        verificationDetailsId: 'P7H84Je9JRow7Q',
        verificationStatus: verificationStatus,
      },
    ],
  },
  {
    name: 'remove_brand_details_field',
    isDisabled: false,
    isRequired: false,
    isHidden: false,
    meta: {
      title: 'null',
      description: 'null',
      defaultValue: 'null',
      size: 'null',
      accessibilityLabel: 'null',
      dataType: 'bool',
      selectionType: 'null',
    },
    failureReason: '',
    failureReasonType: '',
    booleanValue: false,
  },
];
interface Documents {
  name: string;
  size: number;
  fileStoreId: string;
}
export interface GetMockModularResponseProps {
  agreementType?: string;
  agreementComponentStatus?: string;
  agreementSentAt?: string;
  agreementConsentedAt?: string;
  agreementDocuments?: Documents[];
  agreementStatusField?: string;
  retrySentAt?: string;
  vas_cc_emi_rate_field?: string;
  vas_dc_emi_rate_field?: string;
  upi_mdr_rate_field?: string;
  addNachComponent?: boolean;
  acquisition_model_field?: 'aggregator' | 'direct';
  storeType?: 'multi_brand_outlet';
  brandName?: 'vivo';
  brandDataFields?: string[];
  optionalBrandFields?: string[];
  dealerCode?: string;
  stateCode?: string;
  distributorCode?: string;
  verificationStatus?: BRAND_EMI_VERIFICATION_STATUS_ENUM;
}
export const getMockModularResponse = ({
  acquisition_model_field = 'aggregator',
  agreementDocuments = [
    {
      fileStoreId: 'frefer',
      name: 'ferrfer',
      size: 2112,
    },
  ],
  vas_cc_emi_rate_field = '0',
  vas_dc_emi_rate_field = '0',
  upi_mdr_rate_field = '0',
  addNachComponent = true,
  storeType = 'multi_brand_outlet',
  brandName = 'vivo',
  brandDataFields,
  dealerCode,
  stateCode,
  distributorCode,
  verificationStatus,
}: GetMockModularResponseProps) => ({
  id: 'Oe0O6xyCfmHWdv',
  progress: 5,
  status: 'processing',
  milestones: [
    {
      meta: {
        template: 'grid',
        title: 'Merchant Onboarding',
      },
      name: 'sales_milestone',
      progress: 36.71875,
      status: 'processing',
      steps: [
        {
          modularComponents: [
            {
              fields: [
                {
                  failureReason: '',
                  failureReasonType: '',
                  isEditable: true,
                  isHidden: false,
                  isInternal: false,
                  isRequired: true,
                  meta: {
                    dataType: 'radio',
                    default_value: 'aggregator',
                    options: [
                      {
                        label: 'Aggregator Model',
                        value: 'aggregator',
                      },
                      {
                        label: 'Direct Model',
                        value: 'direct',
                      },
                    ],
                    selection_type: 'single',
                    title: 'Select Onboarding Model',
                  },
                  name: 'acquisition_model_field',

                  value: acquisition_model_field,
                },
              ],
              isRequired: true,
              meta: {
                description: 'Choose payment methods & review MDR rates',
                is_custom_pricing_applicable: true,
                template: 'linear',
                title: '3. Payment Method & Service Selection',
              },
              name: 'acquisition_model_component',
              progress: 100,
              status: 'executed',
              verification: null,
            },
            {
              fields: [
                {
                  name: 'custom_rates_enabled_field',
                  isDisabled: false,
                  isRequired: false,
                  isHidden: false,
                  meta: {
                    title: 'Custom rated applicable for merchants',
                    description: null,
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'checkbox',
                    selectionType: null,
                    options: null,
                    validations: null,
                  },
                  failureReason: '',
                  failureReasonType: '',
                  booleanValue: false,
                },
                {
                  name: 'custom_rates_documents_field',
                  isDisabled: false,
                  isRequired: false,
                  isHidden: false,
                  meta: {
                    title: null,
                    description: null,
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'file_upload',
                    selectionType: null,
                    options: null,
                    validations: [
                      {
                        condition: '5 MB',
                        errorMessage: 'File size greater than 5 MB',
                        type: 'maxFileSize',
                      },
                      {
                        condition: '5',
                        errorMessage: 'No of files greater than 5',
                        type: 'maxFiles',
                      },
                    ],
                  },
                  failureReason: '',
                  failureReasonType: '',
                  arrayOfDocumentsUploadValue: agreementDocuments,
                },
                {
                  name: 'previous_custom_rates_documents_field',
                  isDisabled: false,
                  isRequired: false,
                  isHidden: false,
                  meta: {
                    title: null,
                    description: null,
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'file_upload',
                    selectionType: null,
                    options: null,
                    validations: null,
                  },
                  failureReason: '',
                  failureReasonType: '',
                  arrayOfDocumentsUploadValue: [],
                },
                {
                  name: 'debit_card_rupay_mdr_rate_field',
                  isDisabled: false,
                  isRequired: true,
                  isHidden: false,
                  meta: {
                    title: 'Debit Card (Rupay)',
                    description: null,
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'float',
                    selectionType: null,
                    options: null,
                    validations: null,
                  },
                  failureReason: '',
                  failureReasonType: '',
                  stringValue: '0',
                },
                {
                  name: 'debit_card_visa_mastercard_maestro_greater_than_2k_mdr_rate_field',
                  isDisabled: false,
                  isRequired: true,
                  isHidden: false,
                  meta: {
                    title: 'Debit Card (VISA/ MasterCard/ Maestro)',
                    description: 'Greater than 2k transaction amount size',
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'float',
                    selectionType: null,
                    options: null,
                    validations: null,
                  },
                  failureReason: '',
                  failureReasonType: '',
                  stringValue: '0',
                },
                {
                  name: 'debit_card_visa_mastercard_maestro_less_than_2k_mdr_rate_field',
                  isDisabled: false,
                  isRequired: true,
                  isHidden: false,
                  meta: {
                    title: 'Debit Card (VISA/ MasterCard/ Maestro)',
                    description: 'Less than 2k transaction amount size',
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'float',
                    selectionType: null,
                    options: null,
                    validations: null,
                  },
                  failureReason: '',
                  failureReasonType: '',
                  stringValue: '0',
                },
                {
                  name: 'credit_card_mdr_rate_field',
                  isDisabled: false,
                  isRequired: true,
                  isHidden: false,
                  meta: {
                    title: 'Credit Card',
                    description: 'Auto populated as per MCC',
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'float',
                    selectionType: null,
                    options: null,
                    validations: null,
                  },
                  failureReason: '',
                  failureReasonType: '',
                  stringValue: '0',
                },
                {
                  name: 'prepaid_b2b_corporate_channel_international_card_mdr_rate_field',
                  isDisabled: false,
                  isRequired: true,
                  isHidden: false,
                  meta: {
                    title: 'Prepaid/ B2B/ Corporate/ Channel Cards/ International Cards',
                    description: null,
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'float',
                    selectionType: null,
                    options: null,
                    validations: null,
                  },
                  failureReason: '',
                  failureReasonType: '',
                  stringValue: '0',
                },
                {
                  name: 'upi_mdr_rate_field',
                  isDisabled: false,
                  isRequired: true,
                  isHidden: false,
                  meta: {
                    title: 'UPI',
                    description: null,
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'float',
                    selectionType: null,
                    options: null,
                    validations: null,
                  },
                  failureReason: '',
                  failureReasonType: '',
                  stringValue: upi_mdr_rate_field,
                },
                {
                  name: 'vas_cc_emi_rate_field',
                  isDisabled: false,
                  isRequired: false,
                  isHidden: false,
                  meta: {
                    title: 'CC EMI',
                    description: null,
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'float',
                    selectionType: null,
                    options: null,
                    validations: null,
                  },
                  failureReason: '',
                  failureReasonType: '',
                  stringValue: vas_cc_emi_rate_field,
                },
                {
                  name: 'vas_dc_emi_rate_field',
                  isDisabled: false,
                  isRequired: false,
                  isHidden: false,
                  meta: {
                    title: 'DC EMI',
                    description: null,
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'float',
                    selectionType: null,
                    options: null,
                    validations: null,
                  },
                  failureReason: '',
                  failureReasonType: '',
                  stringValue: vas_dc_emi_rate_field,
                },
                {
                  name: 'vas_cc_emi_rate_enabled_field',
                  isDisabled: false,
                  isRequired: true,
                  isHidden: false,
                  meta: {
                    title: null,
                    description: null,
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'checkbox',
                    selectionType: null,
                    options: null,
                    validations: null,
                  },
                  failureReason: '',
                  failureReasonType: '',
                  booleanValue: false,
                },
                {
                  name: 'vas_dc_emi_rate_enabled_field',
                  isDisabled: false,
                  isRequired: true,
                  isHidden: false,
                  meta: {
                    title: null,
                    description: null,
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'checkbox',
                    selectionType: null,
                    options: null,
                    validations: null,
                  },
                  failureReason: '',
                  failureReasonType: '',
                  booleanValue: false,
                },
                {
                  name: 'mdr_vas_pricing_field',
                  isDisabled: false,
                  isRequired: true,
                  isHidden: true,
                  meta: {
                    title: 'MDR VAS Pricing',
                    description: null,
                    defaultValue: null,
                    size: null,
                    accessibilityLabel: null,
                    hideOnReviewScreen: null,
                    dataType: 'custom',
                    selectionType: null,
                    options: null,
                    validations: null,
                  },
                  failureReason: '',
                  failureReasonType: '',
                  jsonValue: {
                    credit_card_mdr_rate_field: null,
                    debit_card_rupay_mdr_rate_field: null,
                    debit_card_visa_mastercard_maestro_greater_than_2k_mdr_rate_field: null,
                    debit_card_visa_mastercard_maestro_less_than_2k_mdr_rate_field: null,
                    prepaid_b2b_corporate_channel_international_card_mdr_rate_field: null,
                    upi_mdr_rate_field: null,
                    vas_cc_emi_rate_field: null,
                    vas_dc_emi_rate_field: null,
                  },
                },
              ],
              isRequired: true,
              meta: {
                defaultValues: {
                  credit_card_mdr_rate_field: '0.5',
                  debit_card_rupay_mdr_rate_field: '1.2',
                  debit_card_visa_mastercard_maestro_greater_than_2k_mdr_rate_field: '0.5',
                  debit_card_visa_mastercard_maestro_less_than_2k_mdr_rate_field: '0.66',
                  prepaid_b2b_corporate_channel_international_card_mdr_rate_field: '0.5',
                  upi_mdr_rate_field: '0.5',
                  vas_cc_emi_rate_field: '1.5',
                  vas_dc_emi_rate_field: '0.5',
                },
                description: 'Choose MDR Rates & Value Added Services',
                is_custom_pricing_applicable: true,
                template: 'linear',
                title: 'Payment Method & Service Selection',
              },
              name: 'mdr_vas_rates_component',
              progress: 37.5,
              status: 'processing',
              verification: null,
            },
            {
              name: addNachComponent ? 'nach_form_component' : '',
            },
            {
              fields: getMockBrandEmiFields({
                dealerCode,
                stateCode,
                distributorCode,
                verificationStatus,
                brandName,
                storeType,
              }),
              meta: {
                description: 'Choose Brand EMI',
                errorCode: null,
                isHidden: null,
                template: 'linear',
                title: '3. Payment Method & Service Selection',
                validations: null,
                defaultValues: null,
                brandDataFields: brandDataFields,
                merchantGstField: null,
                deviceConfig: null,
                metaUi: null,
              },
              progress: 0,
              status: 'pending',
              name: 'brand_emi_component',
            },
          ],
          meta: {
            description: "Provide merchant's business information to start the POS journey",
            icon: 'Pricing Image',
            template: 'linear',
            title: '4. Payment Method & Service Selection',
          },
          name: 'pricing_step',
          progress: 68.75,
          status: 'processing',
        },
      ],
    },
  ],
});

export const mockProps = {
  aggregatorMethodForm: {
    type: PaymentMethodFormType.AGGREGATOR,
    form: {
      [PaymentMethodsFieldKeyNames.DEBIT_CARD_RUPAY_MDR_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
      },
      [PaymentMethodsFieldKeyNames.DEBIT_CARD_VISA_MASTERCARD_MAESTRO_GREATER_THAN_2K_MDR_RATE_FIELD]:
        {
          checked: false,
          value: '1',
          defaultValue: '1',
          isRequired: false,
          isDisabled: false,
          isHidden: false,
          description: '1',
          title: '1',
        },
      [PaymentMethodsFieldKeyNames.DEBIT_CARD_VISA_MASTERCARD_MAESTRO_LESS_THAN_2K_MDR_RATE_FIELD]:
        {
          checked: false,
          value: '1',
          defaultValue: '1',
          isRequired: false,
          isDisabled: false,
          isHidden: false,
          description: '1',
          title: '1',
        },
      [PaymentMethodsFieldKeyNames.CREDIT_CARD_MDR_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
      },
      [PaymentMethodsFieldKeyNames.PREPAID_B2B_CORPORATE_CHANNEL_INTERNATIONAL_CARD_MDR_RATE_FIELD]:
        {
          checked: false,
          value: '1',
          defaultValue: '1',
          isRequired: false,
          isDisabled: false,
          isHidden: false,
          description: '1',
          title: '1',
        },
      [PaymentMethodsFieldKeyNames.UPI_MDR_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
      },
      [PaymentMethodsFieldKeyNames.VAS_CC_EMI_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: true,
        shouldShowValueInput: true,
      },
      [PaymentMethodsFieldKeyNames.VAS_DC_EMI_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: true,
        shouldShowValueInput: true,
      },
      [PaymentMethodsFieldKeyNames.CUSTOM_RATES_DOCUMENTS_FIELD]: {
        checked: false,
        value: [],
        defaultValue: '',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: false,
        shouldShowValueInput: false,
      },
      [PaymentMethodsFieldKeyNames.CUSTOM_RATES_ENABLED_FIELD]: {
        checked: false,
        value: 1,
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: false,
        shouldShowValueInput: false,
      },
      [PaymentMethodsFieldKeyNames.BRAND_EMI_RATE_ENABLED_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: true,
        shouldShowValueInput: false,
      },
      [PaymentMethodsFieldKeyNames.EMI_PLUS_RATE_ENABLED_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: true,
        shouldShowValueInput: false,
      },
      [PaymentMethodsFieldKeyNames.BRAND_EMI_CC_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: false,
        shouldShowValueInput: true,
      },
      [PaymentMethodsFieldKeyNames.BRAND_EMI_DC_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: false,
        shouldShowValueInput: true,
      },
      [PaymentMethodsFieldKeyNames.EMI_PLUS_CC_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: false,
        shouldShowValueInput: true,
      },
      [PaymentMethodsFieldKeyNames.EMI_PLUS_DC_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: false,
        shouldShowValueInput: true,
      },
    },
  },
  directMethodForm: {
    type: PaymentMethodFormType.AGGREGATOR,
    form: {
      [PaymentMethodsFieldKeyNames.VAS_CC_EMI_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: true,
        shouldShowValueInput: true,
      },
      [PaymentMethodsFieldKeyNames.VAS_DC_EMI_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: true,
        shouldShowValueInput: true,
      },
      [PaymentMethodsFieldKeyNames.CUSTOM_RATES_DOCUMENTS_FIELD]: {
        checked: false,
        value: [],
        defaultValue: '',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: false,
        shouldShowValueInput: false,
      },
      [PaymentMethodsFieldKeyNames.CUSTOM_RATES_ENABLED_FIELD]: {
        checked: false,
        value: 1,
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: true,
        shouldShowValueInput: false,
      },
      [PaymentMethodsFieldKeyNames.BRAND_EMI_RATE_ENABLED_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: true,
        shouldShowValueInput: false,
      },
      [PaymentMethodsFieldKeyNames.EMI_PLUS_RATE_ENABLED_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: true,
        shouldShowValueInput: false,
      },
      [PaymentMethodsFieldKeyNames.BRAND_EMI_CC_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: false,
        shouldShowValueInput: true,
      },
      [PaymentMethodsFieldKeyNames.BRAND_EMI_DC_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: false,
        shouldShowValueInput: true,
      },
      [PaymentMethodsFieldKeyNames.EMI_PLUS_CC_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: false,
        shouldShowValueInput: true,
      },
      [PaymentMethodsFieldKeyNames.EMI_PLUS_DC_RATE_FIELD]: {
        checked: false,
        value: '1',
        defaultValue: '1',
        isRequired: false,
        isDisabled: false,
        isHidden: false,
        description: '1',
        title: '1',
        shouldShowCheckBox: false,
        shouldShowValueInput: true,
      },
    },
  },
};

export const getMockModularConfigWithPricingStep = ({
  hasBrandSummaryField = true,
  hasBrandOptions = true,
  hasAcquisitionModelField = true,
  acquisitionModelField = 'aggregator',
  agreementDocuments = [
    {
      fileStoreId: 'frefer',
      name: 'ferrfer',
      size: 2112,
    },
  ],
  vasCcEmiRateField = '0',
  vasDcEmiRateField = '0',
  upiMdrRateField = '0',
  isBrandEmiCcDisabled = false,
  isBrandEmiDcDisabled = false,
  hasBrandEmiCcRateField = true,
  hasBrandEmiComponent = true,
  hasStoreTypeOptions = true,
  hasStdRates = true,
}: any) => ({
  __typename: 'merchantModularOnboardingDetailsSuccessResponse',
  success: true,
  onboardingState: '',
  countryCode: 'IN',
  merchantType: 'merchant',
  onboardingType: 'modular',
  workflowData: {
    id: 'Oe0O6xyCfmHWdv',
    progress: 5,
    status: 'processing',
    milestones: [
      {
        progress: 36.71875,
        meta: {},
        canSubmit: false,
        name: 'sales_milestone',
        status: 'processing',
        steps: [
          {
            __typename: 'ModularOnboardingStepWithModularComponents',
            name: 'pricing_step',
            progress: 100,
            status: 'executed',
            meta: {
              title: '4. Payment Method & Service Selection',
              description: "Provide merchant's business information to start the POS journey",
            },
            modularComponents: [
              {
                fields: [
                  hasAcquisitionModelField && {
                    failureReason: '',
                    failureReasonType: '',
                    isEditable: true,
                    isHidden: false,
                    isInternal: false,
                    isRequired: true,
                    meta: {
                      dataType: 'radio',
                      default_value: 'aggregator',
                      options: [
                        {
                          label: 'Aggregator Model',
                          value: 'aggregator',
                        },
                        {
                          label: 'Direct Model',
                          value: 'direct',
                        },
                      ],
                      selection_type: 'single',
                      title: 'Select Onboarding Model',
                    },
                    name: 'acquisition_model_field',

                    stringValue: acquisitionModelField,
                  },
                ],
                isRequired: true,
                meta: {
                  description: 'Choose payment methods & review MDR rates',
                  is_custom_pricing_applicable: true,
                  template: 'linear',
                  title: '3. Payment Method & Service Selection',
                },
                name: 'acquisition_model_component',
                progress: 100,
                status: 'executed',
                verification: null,
              },
              {
                fields: [
                  {
                    name: 'custom_rates_enabled_field',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: 'Custom rated applicable for merchants',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'checkbox',
                      selectionType: null,
                      options: null,
                      validations: null,
                    },
                    failureReason: '',
                    failureReasonType: '',
                    booleanValue: false,
                  },
                  {
                    name: 'custom_rates_documents_field',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: null,
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'file_upload',
                      selectionType: null,
                      options: null,
                      validations: [
                        {
                          condition: '5 MB',
                          errorMessage: 'File size greater than 5 MB',
                          type: 'maxFileSize',
                        },
                        {
                          condition: '5',
                          errorMessage: 'No of files greater than 5',
                          type: 'maxFiles',
                        },
                      ],
                    },
                    failureReason: '',
                    failureReasonType: '',
                    arrayOfDocumentsUploadValue: agreementDocuments,
                  },
                  {
                    name: 'previous_custom_rates_documents_field',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: null,
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'file_upload',
                      selectionType: null,
                      options: null,
                      validations: null,
                    },
                    failureReason: '',
                    failureReasonType: '',
                    arrayOfDocumentsUploadValue: [],
                  },
                  {
                    name: 'debit_card_rupay_mdr_rate_field',
                    isDisabled: false,
                    isRequired: true,
                    isHidden: false,
                    meta: {
                      title: 'Debit Card (Rupay)',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'float',
                      selectionType: null,
                      options: null,
                      validations: null,
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: '0',
                  },
                  {
                    name: 'debit_card_visa_mastercard_maestro_greater_than_2k_mdr_rate_field',
                    isDisabled: false,
                    isRequired: true,
                    isHidden: false,
                    meta: {
                      title: 'Debit Card (VISA/ MasterCard/ Maestro)',
                      description: 'Greater than 2k transaction amount size',
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'float',
                      selectionType: null,
                      options: null,
                      validations: null,
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: '0',
                  },
                  {
                    name: 'debit_card_visa_mastercard_maestro_less_than_2k_mdr_rate_field',
                    isDisabled: false,
                    isRequired: true,
                    isHidden: false,
                    meta: {
                      title: 'Debit Card (VISA/ MasterCard/ Maestro)',
                      description: 'Less than 2k transaction amount size',
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'float',
                      selectionType: null,
                      options: null,
                      validations: null,
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: '0',
                  },
                  {
                    name: 'credit_card_mdr_rate_field',
                    isDisabled: false,
                    isRequired: true,
                    isHidden: false,
                    meta: {
                      title: 'Credit Card',
                      description: 'Auto populated as per MCC',
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'float',
                      selectionType: null,
                      options: null,
                      validations: null,
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: '0',
                  },
                  {
                    name: 'prepaid_b2b_corporate_channel_international_card_mdr_rate_field',
                    isDisabled: false,
                    isRequired: true,
                    isHidden: false,
                    meta: {
                      title: 'Prepaid/ B2B/ Corporate/ Channel Cards/ International Cards',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'float',
                      selectionType: null,
                      options: null,
                      validations: null,
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: '0',
                  },
                  {
                    name: 'upi_mdr_rate_field',
                    isDisabled: false,
                    isRequired: true,
                    isHidden: false,
                    meta: {
                      title: 'UPI',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'float',
                      selectionType: null,
                      options: null,
                      validations: null,
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: upiMdrRateField,
                  },
                  {
                    name: 'vas_cc_emi_rate_field',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: 'CC EMI',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'float',
                      selectionType: null,
                      options: null,
                      validations: null,
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: vasCcEmiRateField,
                  },
                  {
                    name: 'vas_dc_emi_rate_field',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: 'DC EMI',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'float',
                      selectionType: null,
                      options: null,
                      validations: null,
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: vasDcEmiRateField,
                  },
                  {
                    name: 'vas_cc_emi_rate_enabled_field',
                    isDisabled: false,
                    isRequired: true,
                    isHidden: false,
                    meta: {
                      title: null,
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'checkbox',
                      selectionType: null,
                      options: null,
                      validations: null,
                    },
                    failureReason: '',
                    failureReasonType: '',
                    booleanValue: false,
                  },
                  {
                    name: 'vas_dc_emi_rate_enabled_field',
                    isDisabled: false,
                    isRequired: true,
                    isHidden: false,
                    meta: {
                      title: null,
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'checkbox',
                      selectionType: null,
                      options: null,
                      validations: null,
                    },
                    failureReason: '',
                    failureReasonType: '',
                    booleanValue: false,
                  },
                  hasBrandEmiCcRateField && {
                    name: 'brand_emi_cc_rate_field',
                    isDisabled: isBrandEmiCcDisabled,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: 'Brand EMI Credit Card',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'float',
                      selectionType: null,
                      options: null,
                      validations: null,
                      jsonValue: {
                        data_type: 'float',
                        title: 'Brand EMI Credit Card',
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: null,
                  },
                  {
                    name: 'brand_emi_dc_rate_field',
                    isDisabled: isBrandEmiDcDisabled,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: 'Brand EMI Debit Card',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'float',
                      selectionType: null,
                      options: null,
                      validations: null,
                      jsonValue: {
                        data_type: 'float',
                        title: 'Brand EMI Debit Card',
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: null,
                  },
                  {
                    name: 'mdr_vas_pricing_field',
                    isDisabled: false,
                    isRequired: true,
                    isHidden: true,
                    meta: {
                      title: 'MDR VAS Pricing',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'custom',
                      selectionType: null,
                      options: null,
                      validations: null,
                    },
                    failureReason: '',
                    failureReasonType: '',
                    jsonValue: {
                      credit_card_mdr_rate_field: null,
                      debit_card_rupay_mdr_rate_field: null,
                      debit_card_visa_mastercard_maestro_greater_than_2k_mdr_rate_field: null,
                      debit_card_visa_mastercard_maestro_less_than_2k_mdr_rate_field: null,
                      prepaid_b2b_corporate_channel_international_card_mdr_rate_field: null,
                      upi_mdr_rate_field: null,
                      vas_cc_emi_rate_field: null,
                      vas_dc_emi_rate_field: null,
                    },
                  },
                ],
                isRequired: true,
                meta: {
                  defaultValues: hasStdRates
                    ? {
                        credit_card_mdr_rate_field: '0.5',
                        debit_card_rupay_mdr_rate_field: '1.2',
                        debit_card_visa_mastercard_maestro_greater_than_2k_mdr_rate_field: '0.5',
                        debit_card_visa_mastercard_maestro_less_than_2k_mdr_rate_field: '0.66',
                        prepaid_b2b_corporate_channel_international_card_mdr_rate_field: '0.5',
                        upi_mdr_rate_field: '0.5',
                        vas_cc_emi_rate_field: '1.5',
                        vas_dc_emi_rate_field: '0.5',
                      }
                    : null,
                  description: 'Choose MDR Rates & Value Added Services',
                  is_custom_pricing_applicable: true,
                  template: 'linear',
                  title: 'Payment Method & Service Selection',
                },
                name: 'mdr_vas_rates_component',
                progress: 37.5,
                status: 'processing',
                verification: null,
              },
              hasBrandEmiComponent && {
                fields: [
                  {
                    name: 'store_type_field',
                    isDisabled: false,
                    isRequired: true,
                    isHidden: false,
                    meta: {
                      title: 'Type of Store',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'select',
                      selectionType: null,
                      options: hasStoreTypeOptions
                        ? [
                            {
                              label: 'Multi Brand Outlet',
                              value: 'multi_brand_outlet',
                            },
                            {
                              label: 'Exclusive Brand Outlet',
                              value: 'exclusive_brand_outlet',
                            },
                            {
                              label: 'Large Format Retail',
                              value: 'large_format_retail',
                            },
                          ]
                        : null,
                      validations: [
                        {
                          condition: true,
                          errorMessage: 'Store type is required',
                          type: 'isRequired',
                        },
                      ],
                      jsonValue: {
                        data_type: 'select',
                        options: [
                          {
                            label: 'Multi Brand Outlet',
                            value: 'multi_brand_outlet',
                          },
                          {
                            label: 'Exclusive Brand Outlet',
                            value: 'exclusive_brand_outlet',
                          },
                          {
                            label: 'Large Format Retail',
                            value: 'large_format_retail',
                          },
                        ],
                        title: 'Type of Store',
                        validations: [
                          {
                            condition: true,
                            errorMessage: 'Store type is required',
                            type: 'isRequired',
                          },
                        ],
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: 'multi_brand_outlet',
                  },
                  {
                    name: 'brand_name_field',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: 'Brand Name',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'select',
                      selectionType: null,
                      options: hasBrandOptions
                        ? [
                            {
                              label: 'Aiwa',
                              value: 'aiwa',
                            },
                            {
                              label: 'Bluestar',
                              value: 'bluestar',
                            },
                          ]
                        : null,
                      validations: [
                        {
                          condition: true,
                          errorMessage: 'Brand name is required',
                          type: 'isRequired',
                        },
                      ],
                      jsonValue: {
                        data_type: 'select',
                        options: [
                          {
                            label: 'Aiwa',
                            value: 'aiwa',
                          },
                          {
                            label: 'Bluestar',
                            value: 'bluestar',
                          },
                        ],
                        title: 'Brand Name',
                        validations: [
                          {
                            condition: true,
                            errorMessage: 'Brand name is required',
                            type: 'isRequired',
                          },
                        ],
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: '',
                  },
                  {
                    name: 'dealer_code_field',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: 'Dealer Code',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'string',
                      selectionType: null,
                      options: null,
                      validations: null,
                      jsonValue: {
                        data_type: 'string',
                        title: 'Dealer Code',
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: '',
                  },
                  {
                    name: 'distributor_code_field',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: 'Distributor Code',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'string',
                      selectionType: null,
                      options: null,
                      validations: null,
                      jsonValue: {
                        data_type: 'string',
                        title: 'Distributor Code',
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: '',
                  },
                  {
                    name: 'state_code_field',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: 'State Code',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'string',
                      selectionType: null,
                      options: null,
                      validations: null,
                      jsonValue: {
                        data_type: 'string',
                        title: 'State Code',
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: '',
                  },
                  {
                    name: 'merchant_gst_field',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: 'Merchant GST',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'string',
                      selectionType: null,
                      options: null,
                      validations: null,
                      jsonValue: {
                        data_type: 'string',
                        title: 'Merchant GST',
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: '',
                  },
                  {
                    name: 'merchant_business_name_field',
                    isDisabled: true,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: 'Merchant Business Name',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'string',
                      selectionType: null,
                      options: null,
                      validations: null,
                      jsonValue: {
                        data_type: 'string',
                        title: 'Merchant Business Name',
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: 'Blackpink limited',
                  },
                  {
                    name: 'fetch_fields_for_brand',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: null,
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'string',
                      selectionType: null,
                      options: null,
                      validations: null,
                      jsonValue: {
                        data_type: 'string',
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: 'elanpro',
                  },
                  {
                    name: 'fetch_fields_for_brand_internal',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: null,
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'string',
                      selectionType: null,
                      options: null,
                      validations: null,
                      jsonValue: {
                        data_type: 'string',
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: '',
                  },
                  hasBrandSummaryField && {
                    name: 'brand_details_summary',
                    isDisabled: false,
                    isRequired: true,
                    isHidden: false,
                    meta: {
                      title: 'Brand Details Summary',
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'BrandDetailsSummary',
                      selectionType: null,
                      options: null,
                      validations: null,
                      jsonValue: {
                        data_type: 'BrandDetailsSummary',
                        title: 'Brand Details Summary',
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    addedBrands: [
                      {
                        name: 'elanpro',
                        dealerCode: null,
                        distributorCode: null,
                        stateCode: null,
                        verificationDetailsId: 'PoyJlI9QCcTQAv',
                        verificationStatus: 'failed',
                        merchantGst: null,
                      },
                    ],
                  },
                  {
                    name: 'remove_brand_details_field',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: null,
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'bool',
                      selectionType: null,
                      options: null,
                      validations: null,
                      jsonValue: {
                        data_type: 'bool',
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    booleanValue: true,
                  },
                  {
                    name: 'reset_brand_details_field',
                    isDisabled: false,
                    isRequired: false,
                    isHidden: false,
                    meta: {
                      title: null,
                      description: null,
                      defaultValue: null,
                      size: null,
                      accessibilityLabel: null,
                      hideOnReviewScreen: null,
                      dataType: 'int',
                      selectionType: null,
                      options: null,
                      validations: null,
                      jsonValue: {
                        data_type: 'int',
                      },
                    },
                    failureReason: '',
                    failureReasonType: '',
                    stringValue: '1738090994',
                  },
                ],
                meta: {
                  description: 'Choose Brand EMI',
                  errorCode: null,
                  isHidden: null,
                  template: 'linear',
                  title: '3. Payment Method & Service Selection',
                  validations: null,
                  defaultValues: null,
                  brandDataFields: [],
                  optionalBrandFields: null,
                  merchantGstField: null,
                  deviceConfig: null,
                  metaUi: null,
                  acquirerPreferenceOptions: null,
                },
                progress: 100,
                status: 'executed',
                name: 'brand_emi_component',
              },
              {
                name:'nach_form_component',
              }
            ],
          },
        ],
      },
    ],
  },
});

export const getMockUseOnboardingContext = () => ({
  values: {
    isNewOnboarding: false,
    merchantId: 'PhjLpFaE7tz4cA',
    onboardingSteps: [],
  },
  states: {
    isModularLoading: false,
    isUpdateModularLoading: false,
    isModularFetchError: false,
    isRefetching: false,
    modularConfig: getMockModularConfigWithPricingStep({}),
    isPosEkycAgent: false,
  },
  handlers: {
    getOnboardingProgress: () => ({ totalSteps: 6, totalCompletedSteps: 2 }),
    handleStepClick: () => ({}),
    updateModularConfig: (payload: any) => ({ ...payload }),
    handleProceedToNextComponent: () => ({}),
    refetchModularConfig: () => ({}),
    getStepConfigStepSlug: () => ({}),
  },
});
