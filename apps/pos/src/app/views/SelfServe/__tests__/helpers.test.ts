// Mock the entire LocalStorageUtils module
jest.mock('@libs/shared-utils', () => ({
  setItemInLocalStorage: jest.fn(),
  getItemFromLocalStorage: jest.fn(),
  removeItemFromLocalStorage: jest.fn(),
}));

import * as LocalStorageUtils from '@libs/shared-utils';
import {
  MOCK_ADDRESSES,
...
import { UPDATE_CART_ACTIONS } from 'apps/pos/src/app/views/SelfServe/constants';
import {
...
describe('helpers', () => {
  beforeEach(() => {
    jest.resetAllMocks();
    // Ensure getItem returns empty array by default
    (LocalStorageUtils.getItemFromLocalStorage as jest.Mock).mockReturnValue('[]');
  });
  ...
  test('saveCartInBrowserStorage should trigger store to localstorage with correct cart item', () => {
    ...
    expect(LocalStorageUtils.setItemInLocalStorage).toHaveBeenCalledWith(
  ...
  test('getCartFromLocalStorage should return cart items from localstorage', () => {
    ...
    expect(retainedCartItems).toStrictEqual([PRODUCT]);
  });
...
describe('Address helpers', () => {
  ...
  test('should initialize the delivery address with user details', () => {
    const initialAddress = initializeDeliveryAddresses({ user: MOCK_USER });
    expect(initialAddress[0].type).toBe('default');
  });
});
