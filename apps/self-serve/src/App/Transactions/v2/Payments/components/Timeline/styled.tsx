...
export const IconBackground = styled.div<{ status: string }>`
  ...
  align-items: center;
  background-color: ${({ status, theme }: { status: string; theme: Theme }) =>
    getIconBackgroundColor(status, theme)};
`;
...
export const StyledGradientBox = styled.div`
  ...
  padding: 8px;
  font-size: 12px;
  font-weight: ${({ theme }: { theme: Theme }) => `${theme.typography.fonts.weight.regular}`};
`;
