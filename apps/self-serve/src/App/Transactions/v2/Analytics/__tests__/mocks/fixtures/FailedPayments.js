...
import Failed from 'apps/self-serve/src/App/Transactions/v2/Analytics/EntityAnalytics/FailedPayments';
...
const mockfailedPaymentsData = 234;
...
export const assertFailureData = async () => {
  ...
  await waitFor(() => {
    expect(screen.queryByText(mockfailedPaymentsData)).toBeInTheDocument();
    expect(screen.queryByText(mockfailureInfo.bank.value)).toBeInTheDocument();
    expect(screen.queryByText(mockfailureInfo.customer.value)).toBeInTheDocument();
    expect(
      screen.queryByText(mockfailureInfo.others.value + mockfailureInfo.business.value),
    ).toBeInTheDocument();
  });
};
