...
const CountryCodeInput = ({
  ...
  const [countryData, setCountryData] = useState<typeof countryListData[number]>(() => {
    return (
      countryListData.find((eachCountryData) => eachCountryData.value === dialCode) || defaultCountryData
    );
  });
  ...
  const onItemSelect = useCallback(
    (selectCountryData: typeof defaultCountryData) => {
      onChange?.({
        dialCode: selectCountryData.value,
        value: phoneNumber || '',
      });
      onDialCodeChange?.(selectCountryData.value);
      setCountryData(selectCountryData);
      setFilter('');
  ...
  const renderDropDownItem = useCallback(
    (dropdownCountryData) => (
      <CountryCodeDropDownItem
        key={dropdownCountryData.country}
        onSelect={onItemSelect}
        countryData={dropdownCountryData}
      />
    ),
    [onItemSelect],
  );
        ...
        {isDropdownVisible ? (
          <DropDownItems data-testid="dropdownItems">
            ...
            <SearchResult>
              ...
              {searchResult.length === 0 ? <div className="no-result">No result found</div> : null}
            </SearchResult>
          </DropDownItems>
        ) : null}
      </DropDownMenu>
