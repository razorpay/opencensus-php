...
type withDashboardServerJestConfigType = (
  options: WithDashboardJestConfigs,
) => ReturnType<WithDashboardJestType>;

export const withDashboardServerJestConfig: withDashboardServerJestConfigType = (options) => {
  const { extendServerJestConfig, serverJestOptions } = options || {};
  ...
  const displayName = serverJestOptions.moduleName.includes('@libs/')
    ? serverJestOptions.moduleName.replace('@libs', '').replace('/', ' ').toUpperCase()
    : serverJestOptions.moduleName.toUpperCase().split('-').join(' ');

  if (!rootDir) {
    throw new Error('[@libs/shared-core] Unable to evaluate rootDir.');
  }

  process.env['JEST_SERVER_MODULE_NAME'] = serverJestOptions.moduleName;
  ...
  const dashboardServerJestConfig: Partial<Config> = {
    rootDir,
    ...
    moduleNameMapper: {
      ...
      ...moduleNameMapGenerator(serverJestOptions.moduleName, rootDir),
    },
        ...
        isReportPortalEnabled && [
          '@reportportal/agent-js-jest',
          {
            ...
            attributes: [
              ...
              {
                key: 'Project',
                value: serverJestOptions.moduleName,
              },
            ],
            restClientConfig: {
              timeout: 200000,
  ...
  const consumerJestConfiguration = extendServerJestConfig?.(dashboardServerJestConfig);
