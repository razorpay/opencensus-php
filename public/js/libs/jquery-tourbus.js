        ...
        (function() {
          ...
          module.exports = Bus = (function() {
            ...
            function Bus(el, options) {
              ...
              this.$container = $(
                utils.dataProp(this.rawData.container, this.options.container)
              );
              this.$original.data({
                tourbus: this,
              });
              ...
              this._setupEvents();
              if (
                utils.dataProp(this.rawData.autoDepart, this.options.autoDepart)
              ) {
                this.$original.trigger('depart.tourbus');
              }
              this._log(
                'built tourbus with el',
                el.toString(),
                'and options',
                this.options
              );
            ...
            Bus.prototype.depart = function() {
              ...
              this.currentLegIndex = utils.dataProp(
                this.rawData.startAt,
            ...
            Bus.prototype._configureElement = function() {
              ...
              this.$el.addClass(
                utils.dataProp(this.rawData['class'], this.options['class'])
              );
            ...
            Bus.prototype._log = function() {
              ...
              return console.log.apply(
                console,
                ['TOURBUS ' + this.id + ':'].concat(__slice.call(arguments))
              );
            };
        ...
        (function() {
          ...
          module.exports = Leg = (function() {
            function Leg(options) {
              ...
              this._setupEvents();
              this.bus._log(
                'leg ' + this.index + ' made with options',
                this.options
              );
            }
            ...
            Leg.prototype.render = function() {
              var arrowClass, html;
              arrowClass = this.options.orientation === 'centered'
                ? ''
                : 'tourbus-arrow';
              this.$el.addClass(
                ' ' +
            ...
            Leg.prototype._setupOptions = function() {
              ...
              for ((_i = 0), (_len = dataProps.length); _i < _len; _i++) {
                prop = dataProps[_i];
                _results.push(
                  (this.options[prop] = utils.dataProp(
                    this.rawData[prop],
                    globalOptions[prop]
                  ))
                );
            ...
            Leg.prototype._setupEvents = function() {
              this.$el.on(
                'click',
                '.tourbus-next',
                $.proxy(this.bus.next, this.bus)
              );
              this.$el.on(
                'click',
                '.tourbus-prev',
                $.proxy(this.bus.prev, this.bus)
              );
              return this.$el.on(
                'click',
                '.tourbus-stop',
                $.proxy(this.bus.stop, this.bus)
              );
            };
            ...
            Leg.prototype._configureScroll = function() {
              this.willScroll =
                $.fn.scrollTo && this.options.scrollTo !== false;
            ...
            Leg.prototype._offsets = function() {
              ...
              switch (this.options.orientation) {
                ...
                case 'left':
                  ...
                  offsets.left =
                    this.targetOffset.left - elWidth - this.options.margin;
                  break;
                case 'right':
                  offsets.top = this.targetOffset.top;
                  offsets.left =
                    this.targetOffset.left +
                    this.targetWidth +
                    this.options.margin;
                  break;
                case 'top':
                  offsets.top =
                    this.targetOffset.top - elHeight - this.options.margin;
                  offsets.left = this.targetOffset.left;
                  break;
                case 'bottom':
                  offsets.top =
                    this.targetOffset.top +
                    this.targetHeight +
                    this.options.margin;
                  offsets.left = this.targetOffset.left;
              }
              ...
              if (
                utils.include(
                  this.options.orientation,
                  validOrientations[this.options.align]
                )
              ) {
