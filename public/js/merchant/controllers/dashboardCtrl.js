...
app
  .controller('DashboardAggregationsCtrl', [
    '$scope',
    '$http',
    function($scope, $http) {
      //Aggreagates
      ...
      request.success(function(result) {
        if (result.data) {
          ...
          $scope.aggregations.data.amount = result.data.payment
            ? result.data.payment.total_amount / 100
            : 0;
          $scope.aggregations.data.payments = result.data.payment
            ? result.data.payment.txn_count
            : 0;
          $scope.aggregations.data.refunds = result.data.refund
            ? result.data.refund.txn_count
            : 0;
          $scope.aggregations.data.settlements = result.data.settlement
            ? result.data.settlement.txn_count
            : 0;
        }
      });
  ...
  .controller('DashboardPaymentAggregationsCtrl', [
    ...
    function($scope, $http) {
      ...
      function parsePaymentAggregations(data) {
        var headers = ['CARD', 'EMI', 'NETBANKING', 'WALLET'],
          total = 0,
          result = {};
      ...
      var request = $http.get(
        '/' + $scope.mode + '/analytics/payment/aggregations'
      );
      request.success(function(result) {
        if (result.data) {
  ...
  .controller('DashboardGraphsCtrl', [
    ...
    function($scope, $http, dateFactory, $user, $state) {
      ...
      function initialiseGraphs() {
        ...
        $scope.transactions.options.tooltipOpts.content =
          'Date: %x <br/> Amount: \u20B9%y';
      }
      function generateGraphs() {
        ...
        request.success(function(data) {
          if (data.success) {
            ...
            $scope.successfull.options.xaxis.minTickSize = getMinTickSize(
              $scope.statType
            );
            $scope.transactions.options.xaxis.minTickSize = getMinTickSize(
              $scope.statType
            );
            $scope.refreshGraph = !$scope.refreshGraph;
          }
