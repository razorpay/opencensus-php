  ...
  .controller('ActivationCtrl', [
    ...
    function(
      ...
      $scope.genOperation = function(flag) {
        ...
        if (flag) {
          $scope.data.business_operation_address =
            $scope.data.business_registered_address;
          $scope.data.business_operation_state =
            $scope.data.business_registered_state;
          $scope.data.business_operation_city =
            $scope.data.business_registered_city;
          $scope.data.business_operation_pin =
            $scope.data.business_registered_pin;
        }
      };
      ...
      function getData() {
        ...
        request.success(function(data) {
          ...
          $scope.data.bank_account_number_confirmation =
            $scope.data.bank_account_number;
      ...
      function saveStep(step) {
        ...
        if (step === bankStep) {
          if (
            data.bank_account_number !== data.bank_account_number_confirmation
          ) {
            $scope.alerts[step].addAlert(
              'danger',
              "Bank Account Number doesn't match"
            );
            return;
          }
        ...
        request
          ...
          .error(function() {
            $scope.alerts[step].addAlert(
              'danger',
              'An error occured. Please refresh and retry.',
              true
            );
            $scope.check[step] = false;
          });
      ...
      function saveFile($files, fieldname) {
        ...
        if (allowed_types.indexOf(file.type) <= -1) {
          $scope.fileAlerts[fieldname].addAlert(
            'danger',
            'Invalid filetype. Only jpg, png, pdf files are allowed.',
            true
          );
          return;
        }
        if (file.size > 2000000) {
          $scope.fileAlerts[fieldname].addAlert(
            'danger',
            'Max file size 2 MB. Convert the file to an image before uploading if necessary.',
            true
          );
          return;
        }
      ...
      function submitForm(step) {
        if ($scope.data.agree_terms !== true) {
          $scope.alerts[step].addAlert(
            'danger',
            'You must agree to the terms & conditions to use Razorpay services',
            true
          );
          return;
        }
