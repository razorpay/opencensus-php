        ...
        return {
          ...
          link: function(scope, element, attrs) {
            ...
            attrs.$observe('notBusyDisabled', function(val) {
              var parsed = $parse(val)(scope);
              scope.notBusyDisabled = angular.isDefined(parsed)
                ? parsed
                : false;
            });
            ...
            scope.isBusyFor = function(config, begin) {
              ...
              if (scope[(key = begin ? 'busyWhenName' : 'notBusyWhenName')])
                return config.name == scope[key];
              else if (scope[(key = begin ? 'busyWhenUrl' : 'notBusyWhenUrl')])
                return config.url == scope[key];
            ...
            scope.$on('busy.begin', function(evt, config) {
              if (!scope.busy && scope.isBusyFor(config, true)) {
                ...
                var msgElement = scope.busyMessageElement
                  ? scope.busyMessageElement.clone()
                  : null;
                if (msgElement || scope.busyMessage)
                  element.html('').append(msgElement || scope.busyMessage);

                element
                  .removeClass(scope.busyRemoveClasses)
                  .addClass(scope.busyAddClasses);
            ...
            scope.$on('busy.end', function(evt, config) {
              if (scope.busy && scope.isBusyFor(config)) {
                ...
                element
                  .removeClass(scope.notBusyRemoveClasses)
                  .addClass(scope.notBusyAddClasses);
