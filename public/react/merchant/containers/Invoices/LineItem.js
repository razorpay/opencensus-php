...
export default class InvoiceLineItem extends Component {
  ...
  render() {
    let { fieldName, index, disabled, items, onRemove } = this.props;
    let selectedOption = this.props.invoice_line_items[index];
    let isEmptyRow = !((selectedOption.item_id &&
      selectedOption.item_id !== 'NULL') ||
      selectedOption.name);

    return (
      <tr
        class={`${isEmptyRow ? 'lineItem--empty' : ''} ${disabled ? 'lineItem--disabled' : ''}`}
      >
        <td>
