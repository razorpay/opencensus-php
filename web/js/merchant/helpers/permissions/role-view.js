import rolesList from './roles-list';

const {
  OWNER,
  ADMIN,
  MANAGER,
  OPERATIONS,
  FINANCE,
  SUPPORT,
  RBL_SUPERVISOR,
  RBL_AGENT,
  SELLERAPP,
  SELLERAPP_PLUS,
  AGENT,
  REGISTRATION_LINK_SUPERVISOR,
  REGISTRATION_LINK_AGENT,
  PARTNER,
  PARTNER_AGENT,
  CASHIER,
  STORE_MANAGER,
  MARKETING,
  IT,
} = rolesList;

/*
 * User level map to decide whether a module can be viewed by the give role.
 * */
export default {
  home: [
    OWNER,
    ADMIN,
    MANAGER,
    OPERATIONS,
    FINANCE,
    PARTNER,
    CASHIER,
    STORE_MANAGER,
    MARKETING,
    IT,
  ],
  payments: [
    OWNER,
    ADMIN,
    MANAGER,
    OPERATIONS,
    FINANCE,
    SUPPORT,
    RBL_SUPERVISOR,
    RBL_AGENT,
    AGENT,
    PARTNER,
    CASHIER,
    STORE_MANAGER,
  ],
  orders: [
    OWNER,
    ADMIN,
    MANAGER,
    OPERATIONS,
    FINANCE,
    SUPPORT,
    RBL_SUPERVISOR,
    RBL_AGENT,
    PARTNER,
    CASHIER,
    STORE_MANAGER,
  ],
  refunds: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE, SUPPORT, PARTNER, CASHIER, STORE_MANAGER],
  payments_batch_uploads: [OWNER, ADMIN, MANAGER, OPERATIONS, PARTNER, CASHIER, STORE_MANAGER],
  refunds_batch_uploads: [OWNER, ADMIN, MANAGER, OPERATIONS, PARTNER, CASHIER, STORE_MANAGER],
  success_rate: [OWNER, ADMIN, MANAGER, OPERATIONS],
  settlements: [
    OWNER,
    ADMIN,
    MANAGER,
    OPERATIONS,
    FINANCE,
    SUPPORT,
    RBL_SUPERVISOR,
    AGENT,
    PARTNER,
    CASHIER,
    STORE_MANAGER,
  ],
  invoices: [
    OWNER,
    ADMIN,
    MANAGER,
    OPERATIONS,
    FINANCE,
    SUPPORT,
    SELLERAPP,
    AGENT,
    RBL_SUPERVISOR,
    SELLERAPP_PLUS,
    PARTNER,
  ],
  items: [
    OWNER,
    ADMIN,
    MANAGER,
    OPERATIONS,
    FINANCE,
    SUPPORT,
    SELLERAPP,
    SELLERAPP_PLUS,
    AGENT,
    PARTNER,
  ],
  payment_links: [
    OWNER,
    ADMIN,
    MANAGER,
    OPERATIONS,
    FINANCE,
    SELLERAPP,
    AGENT,
    RBL_SUPERVISOR,
    SELLERAPP_PLUS,
    RBL_AGENT,
    PARTNER,
    CASHIER,
    STORE_MANAGER,
    MARKETING,
    IT,
  ],
  payment_links_batch_uploads: [
    OWNER,
    ADMIN,
    MANAGER,
    OPERATIONS,
    FINANCE,
    AGENT,
    RBL_SUPERVISOR,
    SELLERAPP,
    SELLERAPP_PLUS,
    PARTNER,
    CASHIER,
    STORE_MANAGER,
    MARKETING,
    IT,
  ],
  payment_pages: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE, SELLERAPP, PARTNER, MARKETING, IT],
  payment_handle: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE, SELLERAPP, PARTNER, MARKETING, IT],
  payment_buttons: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE, SELLERAPP, PARTNER, MARKETING, IT],
  subscription_buttons: [
    OWNER,
    ADMIN,
    MANAGER,
    OPERATIONS,
    FINANCE,
    SELLERAPP,
    PARTNER,
    MARKETING,
    IT,
  ],
  accounts: [OWNER, ADMIN, MANAGER],
  marketplace: [OWNER, ADMIN, MANAGER, OPERATIONS, PARTNER, MARKETING, IT],
  subscriptions: [
    OWNER,
    ADMIN,
    MANAGER,
    OPERATIONS,
    FINANCE,
    REGISTRATION_LINK_SUPERVISOR,
    REGISTRATION_LINK_AGENT,
    PARTNER,
  ],
  addons: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE, SUPPORT],
  virtual_accounts: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE],
  qr_codes: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE],
  stores: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE],
  bbps: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE],
  customers: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE, SUPPORT],
  reports: [
    OWNER,
    ADMIN,
    MANAGER,
    OPERATIONS,
    FINANCE,
    RBL_SUPERVISOR,
    AGENT,
    SUPPORT,
    SELLERAPP_PLUS,
    CASHIER,
    STORE_MANAGER,
    MARKETING,
    IT,
  ],
  api_keys: [OWNER, ADMIN],
  profile: [
    OWNER,
    ADMIN,
    MANAGER,
    OPERATIONS,
    FINANCE,
    SUPPORT,
    SELLERAPP,
    RBL_SUPERVISOR,
    RBL_AGENT,
    REGISTRATION_LINK_AGENT,
    REGISTRATION_LINK_SUPERVISOR,
    CASHIER,
    STORE_MANAGER,
    MARKETING,
    IT,
  ],
  trustedbadge: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE],
  add_funds: [OWNER, ADMIN, MANAGER, OPERATIONS],
  profile_gst: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE, SUPPORT, SELLERAPP, AGENT],
  credits: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE],
  activation: [OWNER, ADMIN, MANAGER, PARTNER],
  referrals: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE, SUPPORT],
  team: [OWNER, ADMIN, RBL_SUPERVISOR],
  invitations: [OWNER, RBL_SUPERVISOR],
  webhooks: [OWNER, ADMIN, MANAGER],
  configuration: [OWNER, ADMIN, MANAGER],
  early_settlement: [OWNER, ADMIN],
  applications: [OWNER],
  offers: [
    OWNER,
    ADMIN,
    MANAGER,
    OPERATIONS,
    FINANCE,
    RBL_SUPERVISOR,
    AGENT,
    SELLERAPP,
    PARTNER,
    MARKETING,
    IT,
  ],
  checkoutrewards: [OWNER, ADMIN, MANAGER, OPERATIONS, AGENT],
  developers_console: [OWNER, ADMIN],

  // partner dashboard permissions
  submerchants: [OWNER, ADMIN, MANAGER, PARTNER_AGENT],
  partner_settings: [OWNER, ADMIN, MANAGER],
  earnings: [OWNER, ADMIN, MANAGER],
  partner_applications: [OWNER, MANAGER, ADMIN],
  partner_home: [OWNER],
  loans: [OWNER, ADMIN],
  cash_advance: [OWNER, ADMIN],
  b2b_payments: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE, SUPPORT],
  partner_reports: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE, SUPPORT],
  partner_navlinks: [OWNER, ADMIN, MANAGER, OPERATIONS, FINANCE, SUPPORT, PARTNER_AGENT],

  // optimizer
  optimizer: [OWNER, ADMIN, MANAGER, OPERATIONS],
  provider_details: [OWNER, ADMIN, MANAGER, OPERATIONS],
  ray: [OWNER, ADMIN, MANAGER],
};
