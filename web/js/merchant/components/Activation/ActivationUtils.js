...
function showForOrgs(activation) {
  const selectedBusinessType =
    activation.state.dirty.business_type || activation.props.data.business_type;
  if (activation.props.user?.isUpdatedLiteOnboarding) {
...
function getActivationState(activationData = {}, isUnregisteredBusiness, isNcEligibile = false) {
  ...
  if (activation_status === 'activated' && !merchant.hold_funds) {
    ...
    if (
      ...
      activation_status === 'needs_clarification' ||
      bdd_verification_status === 'needs_clarification'
    ) {
      if (isNcEligibile) {
...
const getAadhaarErrorMessage = (activation) => {
  const verificationErrorCode =
    activation.props.data.verification_error_codes ||
    activation.props.user.verification_error_codes;
