...
const UploadBannerDrawer: React.FC<IUploadBannerDrawer> = ({
          ...
          return uploadStorefrontImage(croppedImageFile).then((croppedRes) => {
            if (croppedRes && croppedRes.success) {
              ...
              const updatedBannerData = bannerData.map((banner) => {
                if (selectedBanner && banner.croppedSrc === selectedBanner.croppedSrc) {
                  return {
                    ...banner,
                    originalSrc: originalUrl,
                    croppedSrc: croppedUrl,
                  };
                }
                return banner;
              });
  ...
  const handleToggle = (val: boolean, banner: IBannerImage) => {
    const updatedBannerData = bannerData.filter(
      (bannerD) => bannerD.croppedSrc !== banner.croppedSrc,
    );
  ...
  const handleConfirmDelete = () => {
    if (selectedBanner) {
      const updatedBannerData = bannerData.filter(
        (bannerD) => bannerD.croppedSrc !== selectedBanner.croppedSrc,
      );
      handleBannerChange(updatedBannerData);
    }
    setSelectedBanner(null);
  ...
  return (
    <>
      {isMobile ? (
        <BottomSheet isOpen={true} onDismiss={handleClose}>
          <BottomSheetHeader title="Add store banner" />
          <BottomSheetBody>{renderBannerSection()}</BottomSheetBody>
        </BottomSheet>
      ) : (
        <PaymentPagesDrawer
          showCloseBtn={false}
      ...
      {openDeleteModal &&
        (isMobile ? (
          <BottomSheet isOpen={true} onDismiss={handleConfirmCancel}>
            <BottomSheetHeader
