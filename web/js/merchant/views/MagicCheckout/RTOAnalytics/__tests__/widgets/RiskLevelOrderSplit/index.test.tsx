...
import { MANUAL_RISK_ORDER_SPLIT_DATA } from 'merchant/views/MagicCheckout/RTOAnalytics/__tests__/mocks/fixtures';
...
const renderApp = ({ state = {}, ...props } = {}) => {
  render(
    <Provider store={storeWithInitialState({ ...INIT_STATE, ...state })}>
      <RiskLevelOrderSplit {...props} />
    </Provider>,
  );
};

describe('testing risk level order split component', () => {
  test('should render properly', () => {
    renderApp();
    expect(screen.getByText(/^RTO risk distribution?/i)).toBeInTheDocument();
  });
  ...
  test('should show some graph', () => {
    const customState = {
      magicRTOAnalytics: {
        manual_risk_order_split: {
          loading: false,
          data: MANUAL_RISK_ORDER_SPLIT_DATA,
