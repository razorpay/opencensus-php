...
const chartPlugins = (orderType) => [
  {
    beforeDraw: function beforeDraw(chart) {
      ...
      const textMainY = height * 0.45;
      ctx.fillText(textMain, textMainX, textMainY);
      ctx.save();

      ctx.restore();
      const subText = orderType;
      ctx.font = `${(height / 12).toFixed(2)}px lato`;
      ctx.fillStyle = 'grey';
      ctx.textBaseline = 'middle';
...
const DoughnutGroup = ({ reasonsList, orderType }) => {
      ...
      {chartsData?.map((chartData) => (
        <div key={chartData?.labels[0]} className="chart-container">
          <div className="doughnut-chart">
            <Doughnut data={chartData} options={chartOptions} plugins={chartPlugins(orderType)} />
          </div>
          <span>{chartData?.labels[0] || chartData?.labels[1]}</span>
        </div>
      ))}
    </div>
  );
