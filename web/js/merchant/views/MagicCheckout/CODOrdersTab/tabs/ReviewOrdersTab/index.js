...
const ReviewOrders = (props) => {
  ...
  useEffect(() => {
    if (fetchCODOrders)
      fetchCODOrders({
        count: 25,
        skip: 0,
        review_status: REVIEW_ORDERS_CATEGORY,
      });
  }, [fetchCODOrders]);
  ...
  useEffect(() => {
    ...
    const newArray = items.map((item) => {
      if (item.review_status && item.review_status !== 'hold') {
  ...
  return (
    <div className="orders-container review-orders-container">
      ...
      <DataTable
        ...
        items={itemsArray}
        customClass="magic-cod-orders-table"
        loading={loading}
        skip={skip}
        count={count}
        paginate={paginate}
...
export default connect(mapStateToProps, mapDispatchToProps)(ReviewOrders);
