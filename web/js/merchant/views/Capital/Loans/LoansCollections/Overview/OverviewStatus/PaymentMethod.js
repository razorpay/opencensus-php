...
function PaymentMethod({
  ...
  const [viaBalance, setViaBalance] = useState(0);
  const [viaNetbanking, setViaNetbanking] = useState(0);
  ...
  const onRepayNow = () => {
    return handleRepayment(payload, loanData, dispatch)
      .then(() => setView(OVERVIEW_STATUS_VIEWS.PAYMENT_SUCCESS))
      .catch(() => {
        setView(OVERVIEW_STATUS_VIEWS.PAYMENT_FAILURE);
      });
  };
          ...
          },
          {
            label: (
              <div className="flex payment-option">
                ...
                <span className="text-xs">Using Razorpay Checkout</span>
              </div>
            ),
            value: 1,
          },
        ]}
        value={viaBalance ? 0 : 1}
  ...
  return (
    <div className="payment-method">
      <div className="flex payment-method-body">
        ...
        <div className="flex right">
          <AsyncBtn.Primary onClick={onRepayNow} disabled={!viaNetbanking && !viaBalance}>
            Repay
          </AsyncBtn.Primary>
