...
describe('Smart Collect', () => {
  ...
  test('should render the alert correctly when fee_bearer is customer', () => {
    ...
    expect(screen.getByText('Smart collect is not available for you')).toBeInTheDocument();
    expect(
      screen.getByText(
  ...
  test('should render the onboarding blocker correctly when fee_bearer is customer and virtual-accounts feature is disabled', () => {
    const reduxState = getInitialReduxState({
      merchant: { fee_bearer: FEE_BEARER_TYPES.CUSTOMER },
    ...
    expect(screen.getByText('Smart Collect')).toBeInTheDocument();
    expect(
      screen.getByText(
