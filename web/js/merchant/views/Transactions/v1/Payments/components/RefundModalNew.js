...
export const isPartialPayment = (props) => {
  ...
  const amountEntered = rupeesToPaise(payable_amount, payment.currency);
...
export const amountValidation = (props) => {
  ...
  if (value < 1 && payment.currency === 'INR') {
    return `Amount can't be less than 1`;
  }

  const amountError = validateAmount(value, null, payment.currency);
  ...
  if (rupeesToPaise(value, payment.currency) > refundableAmount) {
    return (
      `Amount can't be greater than the total Refundable` +
      ` Amount (${paiseToRupees(refundableAmount, payment.currency)}).`
    );
  }
...
class RefundModal extends Component {
  ...
  componentDidUpdate(prevProps) {
    const { payable_amount, payment } = this.props;
  ...
  refund(speedValue, props, partial) {
    ...
    this.setState({}, () => {
      if (!this.state.refundApiInProgress) {
        ...
        const data = {
          amount: rupeesToPaise(props.amount, payment.currency),
              ...
              {transfers.items.length > 0 && (
                <div className="row">
                  <div className="col-xs-12">
                    <div
                      className={`instant-refund-check ${this.state.reversal ? 'focussed' : ''}`}
...
const mapStateToProps = (state) => {
  ...
  return {
    ...
    current_balance: state.payment.current_balance,
