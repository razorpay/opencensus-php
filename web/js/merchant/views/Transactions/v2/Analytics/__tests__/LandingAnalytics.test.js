import { waitFor, screen, userEvent } from 'test-utils';
import {
  renderApp,
  modifyData,
  assertRefetchData,
...
describe('LandingAnalytics', () => {
  beforeEach(() => {
    ...
    modifyData({ type: 'all', kind: 'loading', value: true });
    modifyData({ type: 'all', kind: 'failed', value: false });
  });
  ...
  test('should show failed state when api fails', async () => {
    modifyData({ type: 'all', kind: 'failed', value: true });
    renderApp();
  ...
  ['refunds', 'disputes', 'failedPayments'].forEach((type) => {
    test(`should show failed state when ${type} api fails`, async () => {
      modifyData({ type: 'all', kind: 'loading', value: false });
      modifyData({ type, kind: 'failed', value: true });
      renderApp();
