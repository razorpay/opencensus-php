...
describe('Tracking Functions', () => {
  ...
  test('should call analyticsTrack with correct parameters for trackOverviewDuration function', () => {
    ...
    expect(analyticsTrack).toHaveBeenCalledWith(
      createTrackObject({
        ...
        properties: { overviewDate: 'Last 30 Days', section: 'Overview' },
      }),
    );
  });
  ...
  test('should call analyticsTrack with correct parameters for trackDurationFilter function', () => {
    ...
    expect(analyticsTrack).toHaveBeenCalledWith(
      createTrackObject({
        ...
        properties: { dateRange: 'Last 7 Days', section: TransactionsPagesMap[pathname] },
      }),
    );
  });

  test('should track status filter', () => {
    trackStatusFilter({ status: 'Success', pathname });
    expect(analyticsTrack).toHaveBeenCalledWith(
      createTrackObject({
        objectName: 'Status Dropdown',
        properties: { status: 'Success', section: TransactionsPagesMap[pathname] },
      }),
    );
  });

  test('should track method filter', () => {
    trackMethodFilter({ paymentMethodSelected: 'Card', pathname });
    expect(analyticsTrack).toHaveBeenCalledWith(
      createTrackObject({
  ...
  test('should track search by filter', () => {
    trackSearchByFilter({ searchBy: 'Transaction ID', pathname });
    expect(analyticsTrack).toHaveBeenCalledWith(
      createTrackObject({
        objectName: 'Search By Dropdown',
        properties: { searchBy: 'Transaction ID', section: TransactionsPagesMap[pathname] },
      }),
    );
  });
  ...
  test('should track details page load correctly', () => {
    const trackData = { latestTransactionStatus: 'Success' };
  ...
  test('should track details copy correctly', () => {
    const trackData = { objectName: 'Payment ID', properties: { custom: 'property' } };
  ...
  test('should track details click correctly', () => {
    const trackData = { objectName: 'Issue Refund', properties: { custom: 'property' } };
