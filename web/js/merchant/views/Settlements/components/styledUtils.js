...
export const StyledCard = styled.div(
  ({ theme, customCardStyle }) => `
    ...
    border-left: 1px solid ${theme?.colors?.surface?.border?.subtle?.lowContrast};
    ${customCardStyle}
`,
);
...
export const SummaryHeader = styled.div(
  ({ theme }) => `
    ...
    border-bottom: 1px solid ${theme?.colors?.surface?.border?.subtle?.lowContrast};
...
export const TimeSummary = styled.span(
  ...
  font-weight: ${theme?.typography?.fonts?.weight?.regular};
  padding-right: ${theme?.spacing?.[4]}px;
  color: ${theme?.colors?.action?.text?.primary?.disabled};
  align-items: center;
`,
);
