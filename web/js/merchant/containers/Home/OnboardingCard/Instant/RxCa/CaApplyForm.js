...
const CaApplyForm = (props) => {
    ...
    const onSubmit = (data) => {
        if (data.name.length <= 0 || data.pin_code.length <= 0 || data.email.length <= 0 || data.phone.length <= 0) {
            showNotification({
                type: 'error',
        ...
        props.tracking.trackEvent(window.rzpQ.merchantActions().clicked('dashboard.neopricing_tracker', {
            clicked_on: 'apply',
        ...
        <div className={`ca-apply ${formDisabled && 'disable-ca-form'}`}>
            ...
            <Form onSubmit={onSubmit}>
                ...
                <Input.Check
                    ...
                    fieldLabel={<div className='terms_condition'>I understand the
                    <a className="btn btn-link" href='https://razorpay.com/links/neo-plan-terms-conditions' target="_blank">
...
export default RTracking({ page: 'RXNeoCaApply' })(connect(mapStateToProps, mapDispatchToProps)(CaApplyForm));
