...
const Primary = ({ tracking, updateUser, settings, openModal, closeModal, caAccount, headState, hasAppliedCa }) => {

    const goToXdashboard = () => {
        window.open('https://x.razorpay.com', '_blank');
        tracking.trackEvent(window.rzpQ.merchantActions().clicked('dashboard.neopricing_tracker', {
            clicked_on: 'apply',
    ...
    const handleSuccess = () => {
        ...
        openModal({
            component: (
                <CaApplyAcknowledge onClose={closeModal} />
            ),
            size: 'small',
    ...
    const openCaApplyModal = () => {
        ...
        openModal({
            component: (
                <CaApplyForm onClose={closeModal} onSuccess={handleSuccess} />
            ),
            size: 'small',
        ...
        <div className='ca-primary-card'>
            ...
            <div className='right'>
                {caAccount && caAccount.status === currentAccountStatuses.activated &&
                    <Button.Primary iconAfter='external-link' onClick={goToXdashboard}>Explore Account</Button.Primary>
                }
...
export default RTracking({ page: 'RXNeoCaPrimaryCard' })(connect(null, mapDispatchToProps)(Primary));