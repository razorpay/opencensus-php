...
import { preDefinedDurations } from 'merchant_common/views/Reports/components/Modals/components/DownloadReport/data';
...
describe('Download Custom Reports', () => {
  ...
  test('should open 1st section by default', () => {
    render(<App />);
    expect(screen.getByLabelText('Select Report Field')).toBeInTheDocument();
    expect(screen.queryByLabelText('Save Report As Input')).toBeInTheDocument();
    expect(screen.getByLabelText('Select Format Field')).toBeInTheDocument();
    expect(screen.queryByLabelText('Select Duration Field')).not.toBeInTheDocument();
    expect(screen.queryByLabelText('Custom Switch')).not.toBeInTheDocument();
  });

  test('select report should be preselected when params has the required id in it', () => {
    renderDetailedApp();
    expect(screen.getByText(TEST_CONFIG.name)).toBeInTheDocument();
  });
  ...
  test('should be able to fill data', async () => {
    renderDetailedApp();
    await userEvent.type(screen.getByLabelText('Save Report As Input'), 'Rzp Doc');
    await userEvent.click(screen.getByLabelText('Select Format Field'));
    ...
    await userEvent.click(screen.getByLabelText('Custom Switch'));
    await userEvent.click(screen.getByLabelText('Select Duration Field'));
    await userEvent.click(screen.getByLabelText(preDefinedDurations[0].label));
