...
import { getLineHeight } from '@razorpay/blade-old/src/_helpers/theme';
...
const TextInput = React.forwardRef(
  (
    ...
    const onFocus = useCallback(() => {
      ...
      setTimeout(() => {
        // To ensure no memory leak. State change is triggered only if the component is mounted
        if (mounted.current) {
        ...
        <RelativeContainer position={variant === 'filled'}>
          {!hasAnimatedLabel ? (
            <Label
              ...
              labelLink={labelLink}
              labelLinkHandler={labelLinkHandler}
...
TextInput.propTypes = {
  ...
  autoComplete: PropTypes.oneOf(['on', 'off', 'one-time-code']),
};
