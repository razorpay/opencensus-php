...
test.describe.parallel(
  'Payments transactions (Test Mode) @flow=transactions @project=payments',
  () => {
    test.use({
    ...
    test.describe.parallel('Transactions Landing screen Overview section', () => {
      test('should allow navigate to Refunds, Disputes and Failed Payments ', async ({ page }) => {
        ...
        await expect(page.getByText('Disputes')).toBeVisible();
        await page.getByRole('button', { name: 'Go Back' }).click();
    ...
    test.describe.parallel('Transactions Landing screen Listing section', () => {
      test('should allow filters & search operations', async ({ page }) => {
        ...
        await expect(
          paymentsList.getByRole('button', { name: 'Payment method: All' }),
        ).toBeVisible();
        ...
        await expect(
          page.getByTestId(`entity-item-row-${id}`).getByText('Authorized'),
        ).toBeVisible();
        await expect(
          page.getByRole('cell', {
