...
@elseif (($reason === 'payout_bank_processing') and (empty($processByTime) === false))

    Payout is being processed by the partner bank. Please check the final status after {{Carbon\Carbon::createFromTimestamp($processByTime, 'Asia/Kolkata')->format("dS F Y, h:i A") ?? null}}.

@elseif ($reason === 'payout_bank_processing')

    Payout is being processed by the partner bank. Please check the final status after some time.
