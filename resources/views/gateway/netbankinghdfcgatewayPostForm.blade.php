...
<body onload="document.form1.submit()">
  ...
  <form id="form1" name="form1" action="{{$data['request']['url']}}" method="post" onsubmit="return true;">
    <input type="hidden" name="ClientCode" value="{{$data['request']['content']['ClientCode']}}">
    <input type="hidden" name="MerchantCode" value="{{$data['request']['content']['MerchantCode']}}">
    <input type="hidden" name="TxnCurrency" value="{{$data['request']['content']['TxnCurrency']}}">
    <input type="hidden" name="TxnAmount" value="{{$data['request']['content']['TxnAmount']}}">
    <input type="hidden" name="TxnScAmount" value="{{$data['request']['content']['TxnScAmount']}}">
    <input type="hidden" name="MerchantRefNo" value="{{$data['request']['content']['MerchantRefNo']}}">
    <input type="hidden" name="SuccessStaticFlag" value="{{$data['request']['content']['SuccessStaticFlag']}}">
    <input type="hidden" name="FailureStaticFlag" value="{{$data['request']['content']['FailureStaticFlag']}}">
    <input type="hidden" name="Date" value="{{$data['request']['content']['Date']}}">
    <input type="hidden" name="Ref1" value="{{$data['request']['content']['Ref1']}}">
    @if (isset($data['request']['content']['ClientAccNum']))
      <input type="hidden" name="ClientAccNum" value="{{$data['request']['content']['ClientAccNum']}}">
    @endif
    <input type="hidden" name="DynamicUrl" value="{{$data['request']['content']['DynamicUrl']}}">
    <input type="hidden" name="CheckSum" value="{{$data['request']['content']['CheckSum']}}">
  </form>
